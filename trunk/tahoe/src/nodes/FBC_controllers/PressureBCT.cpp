/* $Id: PressureBCT.cpp,v 1.10 2016-11-05 15:47:03 tdnguye Exp $ */
// created : rjones 2006
...
fndir(2),
fnsd(3),
fDomain(NULL)  // NULL the fDomain variable so tahoe doesn't crash in case of nfaces==0
...
#endif
			if (i == 0) {
			    // In case nfaces==0 , we fudge a geometry of type kQuadrilateral. This probably doesn't matter as fDomain is set to NULL
				geometry = nfaces>0 ? face_geom[0] : Tahoe::GeometryT::kQuadrilateral;
				fnnodes = faces.MinorDim();
				fFaces.Dimension(nfaces, fnnodes);
				fFaces_alias.Alias(fFaces.Length(),fFaces.Pointer());
				// Only if nfaces>0 we create a DomainIntegration object, otherwise Tahoe crashes with no faces present.
				if (nfaces>0) {
					int nip = fnnodes>0 ? fnnodes : 1;
					fDomain = new DomainIntegrationT(geometry, nip, nip); // fnnodes
					fDomain->Initialize();
				}
			}
			else {
...
void PressureBCT:: ComputeVolume(dArray2DT& coord, double& volume, double& area)
{
	volume = 0.0;
	area = 0.0;
	// Only if DomainIntegrationT has been created,we go ahead or else return volume & area as 0
	if (fDomain) {
...
{
	force = 0.0;
	// Only if DomainIntegrationT has been created,we go ahead or else return force as 0
	if (fDomain) {
		DomainIntegrationT domain = *fDomain;
		domain.TopIP();
		const double* wgs = domain.IPWeights();
...
{
	stiffness = 0.0;
	// Only if DomainIntegrationT has been created,we go ahead or else return stiffness as 0
	if (fDomain) {
		DomainIntegrationT domain = *fDomain;
		domain.TopIP();
		const double* wgs = domain.IPWeights();
...
{
    // Only if DomainIntegrationT has been created,we go ahead or else return delV as it is. We could set it to 0 but the code seems to works fine with it not changed
	if (fDomain) {
		DomainIntegrationT domain = *fDomain;
		delV = 0.0;
		domain.TopIP();
