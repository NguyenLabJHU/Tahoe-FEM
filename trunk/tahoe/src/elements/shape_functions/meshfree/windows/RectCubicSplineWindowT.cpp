		...
		int order, double& w, dArrayT& Dw, dSymMatrixT& DDw)
{
  ...
  else      
  {
	...
	wx = 0.0;
      ...
      /* check y-direction */
      if ((ry >= -2) && (ry < -1))
	wy = 1 / (6 * param_n[1]) * (ry + 2) * (ry + 2) * (ry + 2);
      else if ((ry >= -1) && (ry < 0))
	wy = 1 / param_n[1] * (2/3 - ry * ry * (1 + .5 * ry));
      else if ((ry >= 0) && (ry < 1))
	wy = 1 / param_n[1] * (2/3 - ry * ry * (1 - .5 * ry));
      else if ((ry >= 1) && (ry <= 2))
	wy = 1 / (6 * param_n[1]) * (2 - ry) * (2 - ry) * (2 - ry);
      else
	wy = 0.0;
	...
	wy = 0.0;
      ...
      /* check z-direction */
      if ((rz >= -2) && (rz < -1))
	wz = 1 / (6 * param_n[2]) * (rz + 2) * (rz + 2) * (rz + 2);
      else if ((rz >= -1) && (rz < 0))
	wz = 1 / param_n[2] * (2/3 - rz * rz * (1 + .5 * rz));
      else if ((rz >= 0) && (rz < 1))
	wz = 1 / param_n[2] * (2/3 - rz * rz * (1 - .5 * rz));
      else if ((rz >= 1) && (rz <= 2))
	wz = 1 / (6 * param_n[2]) * (2 - rz) * (2 - rz) * (2 - rz);
      else
	wz = 0.0;
