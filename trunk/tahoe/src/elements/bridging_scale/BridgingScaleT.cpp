...
/* accessors */
const int& BridgingScaleT::CurrIP(void) const
{
	return ShapeFunction().CurrIP();
}

/* the coordinates of the current integration points */
void BridgingScaleT::IP_Coords(dArrayT& ip_coords) const
{
...
/* field gradients */
void BridgingScaleT::IP_ComputeGradient(const LocalArrayT& field, 
...
/* restart operations */
void BridgingScaleT::ReadRestart(istream& in)
{
	/* inherited */
...
void BridgingScaleT::WriteRestart(ostream& out) const
{
	/* inherited */
...
void BridgingScaleT::SideSetToFacets(const StringT& block_ID, const iArray2DT& sideset,
...
//       would be easier to check dimensions of facets.
	...
	for (int i = 0; i < num_facets; i++)
	{
		...
		{
			cout << "\n BridgingScaleT::SideSetToFacets: all sides in set must have\n"
...
void BridgingScaleT::FacetGeometry(ArrayT<GeometryT::CodeT>& facet_geometry, 
...
void BridgingScaleT::InitialCondition(void)
{
	/* inherited */
...
/* surface facets */
void BridgingScaleT::SurfaceFacets(GeometryT::CodeT& geometry,
	iArray2DT& surface_facets, iArrayT& surface_nodes) const
{
	...
	{
		cout << "\n BridgingScaleT::SurfaceFacets: only support identical\n";
	...
	for (int i = 0; i < border_elems.Length(); i++)
	{
		...
		if (!found_open)
		{
			cout << "\n BridgingScaleT::SurfaceFacets: error building surface facet list" << endl;
			throw eGeneralFail;
		}	
	}
...
/* surface nodes */
void BridgingScaleT::SurfaceNodes(iArrayT& surface_nodes) const
{
	/* work space */
	AutoArrayT<int> border_nodes;
	iArrayT   border_elems;
	...
	for (int i = 0; i < border_elems.Length(); i++)
	{
		...
		if (!found_open)
		{
			cout << "\n BridgingScaleT::SurfaceNodes: error building surface node list" << endl;
			throw eGeneralFail;
		}	
	}
...
void BridgingScaleT::ApplyTractionBC(void)
{
...
void BridgingScaleT::FormMass(int mass_type, double constM)
{
...
void BridgingScaleT::AddBodyForce(LocalArrayT& body_force) const
{
	if (fBodySchedule)
	{
		int ndof = NumDOF();
...
void BridgingScaleT::FormMa(MassTypeT mass_type, double constM, 
	...
	switch (mass_type)
	{
		...
		case kLumpedMass:
		{
			...
			if (nodal_values)
				...
				cout << "\n BridgingScaleT::FormMa: expecting nodal values for lumped mass" << endl;
				throw eGeneralFail;
			}
				
//TEMP - what to do with ip values?
if (ip_values) {
	cout << "\n BridgingScaleT::FormMa: lumped mass not implemented for ip sources" << endl;
	throw eGeneralFail;
}
...
void BridgingScaleT::EchoBodyForce(ifstreamT& in, ostream& out)
{
	...
	else
	{
		...
		if (!fBodySchedule) {
			cout << "\n BridgingScaleT::EchoBodyForce: could not resolve schedule " 
			     << n_sched + 1 << endl;
			throw eBadInputValue;
		}	
	}
...
void BridgingScaleT::EchoTractionBC(ifstreamT& in, ostream& out)
{
		...
		for (int card=0; card < setsize; card++)
		  {
			...
			if (min < 0 || max > block_data.Dimension())
			  {
			    cout << "\n BridgingScaleT::EchoTractionBC_TahoeII: node numbers\n";
			...
			for (int f=0; f < num_sides; f++)
			  if (num_facet_nodes[side_set(f,1)] != num_nodes)
			    {
			      cout << "\n BridgingScaleT::EchoTractionBC_TahoeII: sides specified\n";
			...
			if (min != max)
			  {
			    cout << "\n BridgingScaleT::EchoTractionBC_TahoeII: cannot determine number of\n"
	...
	if (NumSD() != NumDOF())
	{
		...
		for (int i = 0; i < fTractionList.Length(); i++)
			...
			{
				cout << "\n BridgingScaleT::EchoTractionBC: coordinate system must be\n"
...
void BridgingScaleT::BoundingElements(iArrayT& elements, iArray2DT& neighbors) const
{
	//TEMP - not parallelized
	if (ElementSupport().Size() > 1)
		cout << "\n BridgingScaleT::BoundingElements: not extended to parallel" << endl;
...
bool BridgingScaleT::CheckMaterialOutput(void) const
{
	...
	if (fMaterialList->Length() > 1)
	{
		...
		if (!OK)	
		{
			cout << "\n BridgingScaleT::CheckMaterialOutput: incompatible output\n"
...
void BridgingScaleT::SetTractionBC(void)
{
