/* $Id: tevp2D.cpp,v 1.12 2001-06-04 15:45:09 hspark Exp $ */
...
const dSymMatrixT& tevp2D::s_ij(void)
{
  ...
  if (fRunState == GlobalT::kFormRHS)
  {
    ...
    if (fCriticalStrain == kFluid) {
      ...
      //cout << ContinuumElement().FEManager().StepNumber() << '\n';
      //cout << "Within s_ij()" << '\n';
      cout << CurrIP() << '\n';
      ...
      dtot[1] = dtemp(1,1);
      dtot[2] = 0.0;
      dtot[3] = dtemp(0,1);   // D is symmetric - could take (1,0)
      ...
      fStress3D /= J;
      if (ContinuumElement().FEManager().StepNumber() == 674 | ContinuumElement().FEManager().StepNumber() == 675)
      {
	//cout << CurrIP() << '\n';
	//cout << "Visco stress = " << fStress3D << '\n';
      }
    }
...
double tevp2D::ComputeTemperature(const ElementCardT& element, int ip)
{
  ...
  if (fCriticalStrain == kFluid) {
    if (ContinuumElement().FEManager().StepNumber() == 675)
    {
      //cout << "Within ComputeTemperature" << '\n';
      //cout << CurrIP() << '\n';
    }
  /* Case where fluid model was used */
    dMatrixT temp_stress(3), tempd(3); 
    tempd = GetD();
    temp_stress = ArrayToMatrix(fTempKirchoff);
    //ComputeGradients();
    double wpdot = temp_stress(0,0) * tempd(0,0) + temp_stress(1,1) * tempd(1,1) + 2.0 * Mu_d * tempd(0,1) * tempd(0,1);
...
double tevp2D::ComputeEffectiveStrain(const ElementCardT& element, int ip)
{
  ...
  /* If fluid model is used (ie shear band has formed) */
    ...
    dMatrixT tempd(3);
    tempd = GetD();
    double temp1 = tempd(0,0) * tempd(0,0);
    double temp2 = tempd(1,1) * tempd(1,1);
    double temp3 = tempd(0,1) * tempd(0,1);
