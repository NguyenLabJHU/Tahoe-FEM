...
class ExodusT
{
  public:
	...
	void ReadConnectivities(int block_ID, GeometryT::GeometryCode& geometry_code,
		iArray2DT& connects) const;
	void WriteConnectivities(int block_ID, GeometryT::GeometryCode geometry_code,
  ...
  protected:
	...
	void GetElementName(int elemnodes, GeometryT::GeometryCode geometry_code, 
		StringT& elem_name, int& num_output_nodes) const;

	/* return the geometry code for the given element name */
	GeometryT::GeometryCode ToGeometryCode(const StringT& elem_name) const;

  private:

	enum Dimensions { MAX_QA_REC = 5, MAX_INFO = 5 };
