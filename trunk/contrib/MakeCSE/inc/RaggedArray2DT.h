/*
 * File: RaggedArray2DT.h - TEMPLATE
 *
 * 2D array with arbitrary "row" lengths. NO functions are provided
 * for data retrieval. derived classes should use ShallowRowCopy()
 * to retrieve data and return an appropriate array type.
 *
 */

/*
 * created      : PAK (09/10/98)
 * last modified: PAK (01/26/98)
 */
...
class RaggedArray2DT
{
  public:
	...
	/* shallow copy */
	void ShallowCopy(const RaggedArray2DT& source);
	void ShallowCopy(const nArray2DT<TYPE>& source); // equal row sizes
	...
	void WriteRow(int row, const ArrayT<TYPE>& array);
  	void WriteRow(int row, const TYPE* array);

	/* write rows with numbers to the output stream */
	void WriteNumbered(ostream& out) const;

  	/* data retrieval */
  	void ShallowRowCopy(int row, ArrayT<TYPE>& rowdata) const;
...
template <class TYPE>
inline void RaggedArray2DT<TYPE>::ShallowCopy(const RaggedArray2DT& source)
{
	/* dimensions */
	...
	/* data */
	fPtrs.ShallowCopy(source.fPtrs);
	fData.ShallowCopy(source.fData);
}

template <class TYPE>
void RaggedArray2DT<TYPE>::ShallowCopy(const nArray2DT<TYPE>& source)
{
	/* dimensions */
...
/* write data */
template <class TYPE>
inline void RaggedArray2DT<TYPE>::WriteRow(int row, const TYPE* array)
{
	memcpy(fPtrs[row], array, sizeof(TYPE)*MinorDim(row));
}

template <class TYPE>
inline void RaggedArray2DT<TYPE>::WriteRow(int row, const ArrayT<TYPE>& array)
{
#if __option(extended_errorcheck)
	if (array.Length() != MinorDim(row)) throw eSizeMismatch;
#endif

	WriteRow(row, array.Pointer());
}
...
{
	int width = OutputWidth(fPtrs[0], out);
...
/* data retrieval */
template <class TYPE>
inline void RaggedArray2DT<TYPE>::ShallowRowCopy(int row, 
