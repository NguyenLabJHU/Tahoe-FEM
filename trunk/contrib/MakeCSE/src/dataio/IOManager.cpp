	...
	fEcho (false)
{
	...
	SetOutput(program_name, version, title, input_file, fOutputFormat);
}

IOManager::IOManager(ifstream_x& in, const IOManager& io_man):
	fLog(io_man.fLog),
...
{
	/* construct output formatter */
	SetOutput((io_man.fOutput)->CodeName(), 
...
void IOManager::ReadConnectivity(const int group, GeometryT::GeometryCode & geocode, iArray2DT& connects, iArrayT& elementmap)
{
...
void IOManager::ReadParameters (ifstream_x& in, bool interactive, const StringT& program, const StringT& version)
{
...
// scan parameter file
void IOManager::ReadInputFile(ifstream_x& in)
{
  StringT word1;
  try
    {
...
bool IOManager::ReadWord1 (ifstream_x& in, StringT& word1) const
{
...
void IOManager::Parse (ifstream_x& in, StringT& word1)
{
...
void IOManager::ReadOutputFormat (ifstream_x& in)
{
  in >> fOutputFormat;
  fLog << " Output format . . . . . . . . . . . . . . . . . = " 
       << fOutputFormat << '\n';
...
void IOManager::ReadInputFormat (ifstream_x& in)
{
  int InputFormat;
  in >> fInputFormat;
...
void IOManager::SetOutput(const StringT& program_name, 
	const StringT& version, const StringT& title, const StringT& input_file, 
	IOBaseT::IOFileType output_format)
{	
	...
	switch (output_format)
	  {
	  case IOBaseT::kExodusII:
	    fOutput = new ExodusOutputT(fLog, outstrings);
	    break;
	  case IOBaseT::kTahoe:
	  case IOBaseT::kTahoeII:
	    fOutput = new FE_ASCIIT(fLog, fExternTahoeII, outstrings);
	    break;
	  case IOBaseT::kEnSight:
	    fOutput = new EnSightOutputT (fLog, outstrings, kdigits, false);
	    break;
	  case IOBaseT::kEnSightBinary:
	    fOutput = new EnSightOutputT (fLog, outstrings, kdigits, true);
	    break;
	  case IOBaseT::kAbaqus:
	    fOutput = new AbaqusOutputT (fLog, outstrings, false);
	    break;
	  case IOBaseT::kAbaqusBinary:
	    fOutput = new AbaqusOutputT (fLog, outstrings, true);
	    break;

	  case IOBaseT::kTecPlot:
	    fOutput = new TecPlotOutputT (fLog, outstrings, kdigits);
	    break;
