// file: AbaqusOutputT.cpp

// created      : SAW (05/31/2000)
...
const dArray2DT& e_values)
{
	...
	if (n_labels.Length() > 0)
	  {
	    ArrayT<AbaqusT::VariableKey> nkeys (n_labels.Length());
	    SetRecordKey (n_labels, nkeys);
	    iArrayT nodes_used;
	    nodes_used.ShallowCopy(fElementSets[ID]->NodesUsed());
	    nodes_used++;
	...
	if (e_labels.Length() > 0)
	  {
	    ArrayT<AbaqusT::VariableKey> ekeys (e_labels.Length());
...
void AbaqusOutputT::SetRecordKey (const ArrayT<StringT>& labels, ArrayT<AbaqusT::VariableKey>& keys) const
{
