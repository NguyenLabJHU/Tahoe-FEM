...
void EnSightOutputT::WriteOutput (double time, int ID, const dArray2DT& n_values, const dArray2DT& e_values)
{
  ...
  AutoArrayT<EnSightT::VariableType> vtypes;
...
void EnSightOutputT::WritePart (ostream& geo, EnSightT& ens, int index) const
{
      ...
      iArrayT nodes_used;
      nodes_used.ShallowCopy (fElementSets[index]->NodesUsed());
...
void EnSightOutputT::WriteCoordinates (ostream& geo, EnSightT& ens, const iArrayT& nodes) const
{
  ...
  for (int i=0; i < nodes.Length(); i++)
    local.WriteRow (i, (*fCoordinates)(nodes[i]));
...
void EnSightOutputT::WriteVariable (EnSightT& ens, bool nodal, int ID, const dArray2DT& values, const ArrayT<StringT>& labels, AutoArrayT<StringT>& names, AutoArrayT<StringT>& files, AutoArrayT<EnSightT::VariableType>& vtypes) const
{
  int dof = fCoordinates->MinorDim();
...
bool EnSightOutputT::IsVector (const ArrayT<StringT>& inlabels, int index, StringT& extension, int dof) const
{
  extension = inlabels[index];
    ...
    if ( (strstr (inlabels[index], "_x")) == NULL &&
	 (strstr (inlabels[index], "_X")) == NULL)
      return false;
	...
	if (inlabels.Length() < index+1)
	  return false;
	if ( (strstr (inlabels[index+1], "_y")) == NULL &&
	     (strstr (inlabels[index+1], "_Y")) == NULL)
	  return false;
      }
    if (dof == 3)
      {
	if (inlabels.Length() < index+2)
	  return false;
	if ( (strstr (inlabels[index+2], "_z")) == NULL &&
	     (strstr (inlabels[index+2], "_Z")) == NULL)
	  return false;
      }
