...
// created:       SAW (12/04/1998)
// last modified: PAK (03/25/2000)
...
void ExodusT::ReadConnectivities(int block_ID, GeometryT::GeometryCode& geometry_code,
	iArray2DT& connects) const
{
	...
	if (connects.MajorDim() > 0)
	{
		...
		geometry_code = ToGeometryCode(elem_type);
		...
		ConvertElementNumbering (connects, geometry_code);	
	}
	else
		geometry_code = GeometryT::kNone;
}

void ExodusT::WriteConnectivities(int block_ID, GeometryT::GeometryCode geometry_code,
	const iArray2DT& connects, const iArrayT* elem_map)
{
	...
	StringT elem_type;
	GetElementName(connects.MinorDim(), geometry_code, elem_type, num_output_nodes);
	...
	{
		ConvertElementNumbering (tempconn, geometry_code);
...
void ExodusT::ReadSideSet(int set_ID, int& block_ID, iArray2DT& sides) const
{
	...
	if (sides.MajorDim() > 0)
	{
		...
		iArrayT elements;
		temp.ShallowRowCopy(0, elements);
...
void ExodusT::GetElementName(int elemnodes, GeometryT::GeometryCode geometry_code, 
...
GeometryT::GeometryCode ExodusT::ToGeometryCode(const StringT& elem_name) const
{
	char *elem_names[7] = {
		"CIRCLE",
		"SPHERE",
		...
		"TETRA",
		"WEDGE"};
	GeometryT::GeometryCode geom_codes[7] = {
		GeometryT::kPoint,
		GeometryT::kPoint,
		GeometryT::kTriangle,
	...
	GeometryT::GeometryCode code = GeometryT::kNone;
...
/* dummies for ExodusII functions */
#ifndef __ACCESS__
