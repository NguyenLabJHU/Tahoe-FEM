...
void AbaqusInputT::ReadConnectivity (int group, GeometryT::GeometryCode& geocode, iArray2DT& connects, iArrayT& elementmap)
{
  int count = 0;
  int numelemnodes;
...
void AbaqusInputT::ReadLabels (ArrayT<StringT>& nlabels, ArrayT<StringT>& elabels, int group_id)
{
  ...
  ArrayT<AbaqusT::VariableKey> nkeys, ekeys;
...
void AbaqusInputT::SetLabelName (AbaqusT::VariableKey key, int& unknown, StringT& name, char incrementor) const
{
  switch (key)
    {
