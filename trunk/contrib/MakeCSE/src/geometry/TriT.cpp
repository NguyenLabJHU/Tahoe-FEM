	...
	dArrayT& weights)
{
	...
	/* dimension checks */
	if (numnodes < 3 || numnodes > 6) throw eGeneralFail;
	if (numint != 1 && 
	    numint != 4 && 
	    numint != 6) throw eGeneralFail;
	if (nsd != kTrinsd) throw eGeneralFail;
...
void TriT::SetExtrapolation(dMatrixT& extrap)
{
	/* dimensions */
...
// TEMP: not implemented with midside nodes
	...
	if (facet < 0 || facet > 2) throw eOutOfRange;
#endif
...
void TriT::FacetGeometry(ArrayT<GeometryCode>& facet_geom, 
