%&LaTeX
% $Id: material.solid.tex,v 1.1 2001-03-13 02:20:52 paklein Exp $
...
\hline
80 & \parbox[c]{4.5in}{\raggedright \textsf{ABAQUS/Standard~\cite{ABAQUSNNNN} UMAT} BCJ}\\
\hline
\end{longtable}
\end{center}
\end{table}

\temp{table of material models and codes???}

\temp{mention that all materials support both implicit and explicit 
analysis. All models implement consistent tangent matrix that should 
produce locally quadratic convergence when used with Newton-type,
...
the input file~\ref{time.section}. The second parameter defines the 
volume change as a percentage. Thermal expansion is disabled by 
specifying a value of 0 for the schedule number. \temp{mention that 
thermal strains are imposed differently for small strain and finite 
strain material models. For small strains, an additive decomposition 
...
\end{equation}
\temp{for finite strain, a multiplicative split of the deformation 
gradient is assumed}
\begin{equation}
...
\begin{figure}[h]
\centerline{\includegraphics[scale = 1.0]{\dirfilepath{\figpath}{C-Bcell.eps}}}
\hangcaption
...
\temp{print tables of modulus values and units.}
...
\begin{figure}[h]
\centerline{\includegraphics[scale = 0.80]{\dirfilepath{\figpath}{DCunitcell.eps}}}
\hangcaption
...
the lattice. Figure~\ref{fig.SWDCbonds} shows the arrangement
of bonds for the Stillinger-Weber potentials. Without the action of the
three-body potentials, this crystal structure does not display any
stiffness to shear deformations. The two face-centered cubic sub-lattices
are denoted by labels $A$ and $B$. For a range of deformations around the
...
=
   ...
   \end{cases}
\end{multline}
where the geometry is detailed in Figure~\ref{fig.SWDCbonds}. Both potentials make 
...
\temp{with and withouth internal equilibration.}
...
\temp{show results for the fracture energy, cohesive strength, and 
modulus in terms of potential, and then show specific results for the
Smith-Ferrante potential. Is the potential selectable?}

Unlike the constitutive models based on the
Cauchy-Born rule for crystalline
materials developed in Chapter~\ref{ch.CBmodels}, implementations
of the VIB model may suffer significant numerical errors
when evaluating integrals of the bond
distribution to determine the strain energy density~\eqref{eq.4.8}, 
stress response~\eqref{eq.4.11}, and modulus~\eqref{eq.4.12}.
With the models for single crystal materials, the number and
orientation of the bonds in the representative volume may be captured exactly
in the numerical implementation. Therefore, the behavior of the model in numerical
simulation will be essentially the same as the theoretically predicted model
behavior. Because of the errors associated with evaluating integrals
over the bond density, this character is not preserved in extension
of the Cauchy-Born rule to the VIB model.
...
the integration procedure. As discussed in
Section~\ref{sec.isoelastic}, an isotropic stored energy function
${\cal W}$ displays a response which is invariant to rigid body
rotations of the undeformed configuration before undergoing a
particular deformation. This condition is expressed by the relation
\begin{equation}
{\cal W} \farg{\mathbf{F}} =
{\cal W} \farg{\mathbf{F} \, \mathbf{R}^{-1}} 
  \tag{\ref{eq.b.0.e}},
\end{equation}
where $\mathbf{F}$ is the deformation gradient and $\mathbf{R}$ is
a proper orthogonal transformation. For the plane stress, isotropic
VIB model~\eqref{eq.4.23}, the integral of the volumetric bond density 
...
requires evaluating the VIB integrals over a sphere~\eqref{eq.4.20}.
...
{Integration point arrangements in
   ...
   \temp{get picture of lat-long points.}}
\end{figure}
...
\temp{However, the model will not display the modulus, fracture
...
\temp{This is for A356}

\subsubsection{2D input parameters}
The parameters for using the 
\temp{same as for three-dimensional analysis. Since this model is 
...
\subsubsection{Output values}
\temp{output values determined by the implemenation of the 
