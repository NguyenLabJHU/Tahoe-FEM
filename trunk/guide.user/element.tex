%&LaTeX
% $Id: element.tex,v 1.2 2001-03-26 09:12:19 paklein Exp $
...
\temp{mention the rational behind the numbering\\
vetrex nodes then midside nodes\\
numbering of nodes on 3D elements makes outward normal\\
facet numbering for 3D domains chosen arbitrarily}
...
\tahoe supports a number of solid continuum elements.  Table ?? lists the
shapes and the number of integration points supported.  Internal element
numbering follows the convention of \textsf{ABAQUS}~\cite{abaqus} 
and \textsf{EnSight}~\cite{EnSight6}.  The
numbering of individual facets is slightly different from Abaqus, in that
element normals will point outward.  The numbering convention used in \tahoe
is shown in Figure ??.  All \tahoe style database files use this convention. 
Differences between the \tahoe numbering scheme and other databases are
accounted for when reading or writing databases.
...
proposed by Gingold and Monaghan\cite{Gingold1977}.
...
\temp{mention that \tahoe allows you to pick\\
the window function\\
basis functions}
...
1 & $\overline{\mathbf{X}}$ 
  ...
  & \parbox[b]{2.0in}{\raggedright \texttt{xc\_[\textsl{i}]} 
    \texttt{\textsl{i}} = 0, \ldots, $n_{\textit{sd}}$  }\\
\hline
2 & $\phi$ 
