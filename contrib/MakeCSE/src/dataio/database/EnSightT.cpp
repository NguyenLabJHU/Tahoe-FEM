...
// created:       SAW (05/13/99)
// last modified: SAW (04/11/00)
...
void EnSightT::WriteCoordinateMap (ostream& fgeo, const iArrayT& nodesmap) const
{
  if (fBinary)
    ...
    nodesmap.PrintWithFormat(fgeo, iwidth, 0, 1, 0);
}
...
int EnSightT::WriteConnectivityHeader (ostream& fgeo, GeometryT::GeometryCode code, int numelems, int numelemnodes) const
{
  StringT elementname;
...
void EnSightT::WriteConnectivityMap (ostream& fgeo, const iArrayT& elementmap) const
{
  if (fBinary)
    ...
    elementmap.PrintWithFormat (fgeo, iwidth, 0, 1, 0);
}
...
void EnSightT::WriteConnectivity (ostream& fgeo, int numelemnodes, const iArray2DT& connects) const
{
  if (fBinary)
    ...
    connects.PrintWithFormat (fgeo, iwidth, 0, connects.MinorDim(), 0);
}
...
void EnSightT::WriteVariableLabels (ostream& fvar, const ArrayT<StringT>& labels, const ArrayT<StringT>& filenames, const ArrayT<EnSightT::VariableType>& t) const
{
...
void EnSightT::GetElementName (StringT& fElementName, int& num_output_nodes, GeometryT::GeometryCode geocode) const
{
  ...
  switch (geocode)
    ...
    default:
      cout << "\nEnSightT::GetElementName cannot find name for Geometry Code " 
	   << geocode << "\n\n";
      fOut << "\nEnSightT::GetElementName cannot find name for Geometry Code " 
	   << geocode << "\n\n";
      throw eGeneralFail;
...
bool EnSightT::CaseFile (ifstream_x& in, StringT& geofile) const
{
...
bool EnSightT::ReadVariableSection (ifstream_x& in, AutoArrayT<StringT>& nlabels, AutoArrayT<StringT>& elabels, AutoArrayT<bool>& nvector, AutoArrayT<bool>& evector, bool filename) const
{
...
bool EnSightT::ReadTimeSection (ifstream_x& in, int& start, int& increment, dArrayT& timesteps) const
{
  char c = 'c';
  int timeset, num;
...
void EnSightT::ReadConnectivity (istream& in, iArray2DT& conn, iArrayT& map, bool elemmapgiven, GeometryT::GeometryCode& code) const
{
  StringT line (81);
...
bool EnSightT::GeometryCode (GeometryT::GeometryCode& code, const StringT& name, int& numelemnodes) const
{
