...
void EnSightInputT::ReadConnectivity (int group, GeometryT::GeometryCode& geocode, iArray2DT& connects, iArrayT& elementmap)
{ 
  ScanGeometryFile();
  ifstream in (fGeometryFile);
  int id = -1;
  ...
  if (fPartDimensions.Length() == 0)
      ...
      while (fData.ReadPart (in, partID))
	{
	  ...
	  if (length == 0)
	    ...
	    fPartDimensions.Resize (length + 1, 0, true);
