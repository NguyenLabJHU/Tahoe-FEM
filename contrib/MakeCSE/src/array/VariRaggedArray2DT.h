/*
 * File: VariRaggedArray2DT.h
 *
 * VariRaggedArray2DT with dynamics resizing functions
 */

/*
 * created      : PAK (02/17/2000)
 * last modified: PAK (02/17/2000)
 */
...
VariRaggedArray2DT<TYPE>::VariRaggedArray2DT(int majordim, int minordim, int blocksize):
	RaggedArray2DT(majordim, minordim, blocksize)
	fPtrs_wrap(15, fPtrs),
	fData_wrap(15, fData)
{
...
inline void VariRaggedArray2DT<TYPE>::Copy(const AutoFill2DT<TYPE>& source)
{
	...
	/* inherited */
	RaggedArray2DT<TYPE>::oCopy(source);
}
	...
	const ArrayT<TYPE*>& data)
{
	...
	/* inherited */
	RaggedArray2DT::Copy(rowcounts, data);
}
...
void VariRaggedArray2DT<TYPE>::CopyCompressed(const AutoFill2DT<TYPE>& source) // removes empty rows
{
	...
	/* inherited */
	RaggedArray2DT::CopyCompressed(rowcounts, data);
}
...
/* write data */
template <class TYPE>
void VariRaggedArray2DT<TYPE>::WriteRow(int row, const ArrayT<TYPE>& array) // with resizing
{
	int old_minor_dim = MinorDim(row);
	if (array.Length() == old_minor_dim)
		/* inherited */
		RaggedArray2DT<TYPE>::WriteRow(row, array.Pointer());
	else
	{
		/* dimensions */
	...
	if (fPtrs.First() != fData.Pointer() ||
	    fPtrs.Last() - fPtrs.First() != fData.Length())
	{
		cout << "\n VariRaggedArray2DT<TYPE>::WriteRow: pointer reassignment error" << endl;
		throw eGeneralFail;
	}
#endif
}
...
template <class TYPE>
inline void VariRaggedArray2DT<TYPE>::WriteRow(int row, const TYPE* array) // no resizing
{
	/* inherited */
	RaggedArray2DT<TYPE>::WriteRow(row, array);
}
