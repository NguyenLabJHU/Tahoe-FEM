...
template <class TYPEPtr> 
pArrayT<TYPEPtr>::pArrayT(int length) { Allocate(length); }

template <class TYPEPtr> 
pArrayT<TYPEPtr>::pArrayT(const pArrayT& source) { operator=(source); }
