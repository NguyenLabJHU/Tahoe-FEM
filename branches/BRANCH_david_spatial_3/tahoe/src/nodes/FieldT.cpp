/* $Id: FieldT.cpp,v 1.47.2.1 2005-07-25 02:37:21 paklein Exp $ */
#include "FieldT.h"
...
{
	const char caller[] = "FieldT::InitialCondition";
	...
	{
		const KBC_CardT& kbc = fKBC[i];
		if (kbc.Mode() == KBC_CardT::kNode)
			nIntegrator().ConsistentKBC(*this, kbc);
		else if (kbc.Mode() == KBC_CardT::kSet)
		{
			ModelManagerT& model_manager = fFieldSupport.ModelManager();
			const iArrayT& node_set = model_manager.NodeSet(kbc.ID());
			nIntegrator().ConsistentKBC(*this, kbc, &node_set);
		}
...
{
	const char caller[] = "FieldT::InitStep";
	...
	/* apply KBC cards */
	for (int i = 0; i < fKBC.Length(); i++) {
		const KBC_CardT& card = fKBC[i];
		if (card.Mode() == KBC_CardT::kNode) /* node-by-node */
			integrator.ConsistentKBC(*this, card);
		else if (card.Mode() == KBC_CardT::kSet) /* by nodeset */
		{
			ModelManagerT& model_manager = FieldSupport().ModelManager();
			const iArrayT& nodes = model_manager.NodeSet(card.ID());
			integrator.ConsistentKBC(*this, card, &nodes);
		}
		else ExceptionT::GeneralFail(caller);
	}
	...
	if (num_IC > 0 || num_KBC > 0 || num_FBC > 0)
	{
		...
		for (int i = 0; i < subs.Length(); i++)
		{
			...
			if (name == "initial_condition") {
				...
				/* IC for sets from file */
				if (node_ID && sub.FindParameter("ic_file") && value == 0.0) 
				{
				...
				/* set card */
				fKBC[num_KBC++].SetValues(node_ID, dof, code, schedule, value);
			}
			else if (name == "force_BC") {
...
{
	const char caller[] = "FieldT::SetBCCode";

	/* check */
	if (card.DOF() < 0 || card.DOF() >= fEqnos.MinorDim())
		ExceptionT::OutOfRange(caller, "dof %d != [0,%d]", 
			card.DOF(), fEqnos.MinorDim()-1);

	/* mark equation */
	if (card.Mode() == KBC_CardT::kNode)
		fEqnos(card.Node(), card.DOF()) = kPrescribed;
	else if (card.Mode() == KBC_CardT::kSet)
	{
		ModelManagerT& model_manager = FieldSupport().ModelManager();
		const iArrayT& nodes = model_manager.NodeSet(card.ID());
		for (int i = 0; i < nodes.Length(); i++)
			fEqnos(nodes[i], card.DOF()) = kPrescribed;
	}
	else ExceptionT::GeneralFail(caller);
}
