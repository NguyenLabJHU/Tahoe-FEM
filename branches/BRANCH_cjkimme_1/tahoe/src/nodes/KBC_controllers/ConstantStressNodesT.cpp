/* $Id: ConstantStressNodesT.cpp,v 1.1.2.1 2003-09-18 21:03:44 cjkimme Exp $ */
#include "ConstantStressNodesT.h"
...
/* constructor */
ConstantStressNodesT::ConstantStressNodesT(NodeManagerT& node_manager, BasicFieldT& field):
...
ConstantStressNodesT::~ConstantStressNodesT(void)
{
   // nothing yet
}

void ConstantStressNodesT::WriteParameters(ostream& out) const
{
	/* inherited */
...
void ConstantStressNodesT::Initialize(ifstreamT& in)
{
...
void ConstantStressNodesT::InitStep(void)
{
	/* really bad, this */
/*	if (qIConly)
	{
	...
	{
		fIncCt++;
		if (fIncCt == fIncs)*/ // time to change pressure
		/*{
			fIncCt = 0;
			SetBCCards();
		}
...
void ConstantStressNodesT::InitialCondition(void)
{
/*	if (!qIConly)
	{
		fP_0 = fTempScale*fTempSchedule->Value();
	}
*/	
...
void ConstantStressNodesT::SetBCCards(void)
{
	/* number of scaled nodes */
//	int n_scaled = 0; 
//	int ndof = fField.NumDOF();
...
void ConstantStressNodesT::Update(const dArrayT& update)
{
#pragma unused(update)
  // Get number from thermostat. Adjust velocities
	double chi = ConstrainedPressureT::VelocityCorrector();
