/* $Id: XML_Attribute_FormatterT.cpp,v 1.4.2.1 2003-04-27 22:20:00 paklein Exp $ */
...
bool XML_Attribute_FormatterT::InitDescriptionFile(ostream& out) const
{
	...
	out << '\n'; 
	out << "<!--Generated by Tahoe::XML_Attribute_FormatterT $Revision: 1.4.2.1 $-->";
	return true;
}
...
bool XML_Attribute_FormatterT::DoWriteDescription(ostream& out, const ParameterListT& list, BinaryTreeT<StringT>& tags) const
{
	...
	if (!list.Inline())
	{
		...
		if (!tags.InsertUnique(list.Name())) {
			cout << "\n " << caller << ": skipping repeated tag: " << list.Name() << endl;
			return false;
		}
			...
			for (int i = 0; i < nested_lists.Length(); i++)
			{
				...
				else /* inlined list */
				{
					...
					for (int j = 0; j < group_nested_lists.Length(); j++) {
						...
						out << group_nested_lists[i].Name() << occur_char(group_list_occur[i]);
					}
					...
					for (int j = 0; j < group_references.Length(); j++) {
						...
						out << group_references[i] << occur_char(group_ref_occur[i]);
					}
