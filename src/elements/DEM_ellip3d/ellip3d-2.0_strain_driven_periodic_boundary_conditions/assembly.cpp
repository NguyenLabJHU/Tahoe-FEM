...
void assembly::findContact() { 
  ...
  int i, j;
  vec u, v;
  num = totalParticleVec.size();
  ot = totalParticleVec.begin();
...
#pragma omp parallel num_threads(NUM_THREADS) private(tid, ts, tnum, it, pt, i, j, u, v) shared(num) reduction(+: possContact)
  {
    ...
    for (j = 0 ; j < tnum; ++j, ++it) { 
      ...
      for (pt = it, ++pt; pt != totalParticleVec.end(); ++pt){
...
#pragma omp parallel num_threads(NUM_THREADS) private(tid, ts, start, end, i, j, u, v) shared(num) reduction(+: possContact)
  {
    ...
    // explore each partition
    for (i = start; i <= end; ++i) { 
      u = totalParticleVec[i]->getCurrPosition();
      for (j = i + 1; j < num; ++j) {
	v = totalParticleVec[j]->getCurrPosition();
	if (   ( vfabs(v-u) < totalParticleVec[i]->getA() + totalParticleVec[j]->getA() )
	       && ( totalParticleVec[i]->getType() !=  1 || totalParticleVec[j]->getType() != 1  )      // not both are fixed particles
	       && ( totalParticleVec[i]->getType() !=  5 || totalParticleVec[j]->getType() != 5  )      // not both are free boundary particles
	       && ( totalParticleVec[i]->getType() != 10 || totalParticleVec[j]->getType() != 10 )  ) { // not both are ghost particles
	  contact<particle> tmpct(totalParticleVec[i], totalParticleVec[j]); // a local and temparory object
	  ++possContact;
	  if(tmpct.isOverlapped())
#pragma omp critical
	    ContactVec.push_back(tmpct);    // containers use value semantics, so a "copy" is pushed back.
...
#pragma omp parallel num_threads(NUM_THREADS) private(tid, ts, i, j, k, u, v) shared(num) reduction(+: possContact)
  {
	...
	u = totalParticleVec[k]->getCurrPosition();
	for (j = k + 1; j < num; ++j) {
	  v = totalParticleVec[j]->getCurrPosition();
	  if (   ( vfabs(v-u) < totalParticleVec[k]->getA() + totalParticleVec[j]->getA() )
		 && ( totalParticleVec[k]->getType() !=  1 || totalParticleVec[j]->getType() != 1  )      // not both are fixed particles
		 && ( totalParticleVec[k]->getType() !=  5 || totalParticleVec[j]->getType() != 5  )      // not both are free boundary particles
		 && ( totalParticleVec[k]->getType() != 10 || totalParticleVec[j]->getType() != 10 )  ) { // not both are ghost particles
	    contact<particle> tmpct(totalParticleVec[k], totalParticleVec[j]); // a local and temparory object
	    ++possContact;
	    if(tmpct.isOverlapped())
#pragma omp critical
	    ContactVec.push_back(tmpct);    // containers use value semantics, so a "copy" is pushed back.
...
void assembly::findContact(){ // serial version, O(n x n), n is the number of particles.
    ...
    vec u,v;
    for (it=totalParticleVec.begin();it!=totalParticleVec.end();++it){
	u=(*it)->getCurrPosition();
	for (pt=it,++pt;pt!=totalParticleVec.end();++pt){
	    v=(*pt)->getCurrPosition();
	    if (   ( vfabs(v-u) < (*it)->getA() + (*pt)->getA())
...
void assembly::findContact(){ // serial version, binning methods, cell slightly larger than maximum particle
  ...
  for (int i = 0; i < nx; ++i)
	...
	z1 = z0 + dz * k;
	z2 = z0 + dz * (k + 1);
	for (int pt = 0; pt < totalParticleVec.size(); ++pt) {
	  center = totalParticleVec[pt]->getCurrPosition();
	  if (center.getx() >= x1 && center.getx() < x2 &&
	      center.gety() >= y1 && center.gety() < y2 &&
	      center.getz() >= z1 && center.getz() < z2)
	    cellVec[i][j][k].second.push_back( totalParticleVec[pt] );
	}
      }
...
*/
	...
	// don't need to update boundary, since periodic boundary conditions are applied and 
	// it is now a strain-driven problem 
//	updateRB(min,minctl,2);
//	updateRB(mid,midctl,2);
//	updateRB(max,maxctl,2);
//	updateRB6();
...
void assembly::unixialCompressionPB2(int   total_steps,  
			REAL topDispl,
			REAL botDispl,
			int   first_snapshot,
			int   snapshots, 
...
//	rigidBoundaryForce(bdry_penetr, bdry_cntnum);
	...
	updateParticle();
	    ...
	    tmp_pos = (*it)->getCurrPosition();
	    tmp_pos1 = (*it)->getPrevPositionPB();	// position at last time step
	    if(tmp_pos.getz()>Zmax-0.5*cellSize){	// top boundary particles
		(*it)->setPrevPositionPB(tmp_pos);
		(*it)->setCurrPosition(vec(tmp_pos.getx(),tmp_pos.gety(),tmp_pos.getz()-topCompressRate));
		(*it)->setPrevPosition(vec(tmp_pos.getx(),tmp_pos.gety(),tmp_pos.getz()-topCompressRate));
	        (*it)->GlobCoef();	// this is very important, since coef[] is based on curr_position
	    }
	    ...
	    if(tmp_pos.getz()<Zmin+0.5*cellSize){	// bot boundary particles
	        (*it)->setPrevPositionPB(tmp_pos);
		(*it)->setCurrPosition(vec(tmp_pos.getx(),tmp_pos.gety(),tmp_pos.getz()+botCompressRate));
		(*it)->setPrevPosition(vec(tmp_pos.getx(),tmp_pos.gety(),tmp_pos.getz()+botCompressRate));
	        (*it)->GlobCoef();	// this is very important, since coef[] is based on curr_position
	    }
