...
// qhull can be refered to "http://www.qhull.org/" and paper "The quickhull algorithm for convex hulls"
...
// complementary area vector in equation (2)
...
// get volume of cell, following equation (34) and (35)
...
// calculate spatial velocity gradient tensor for each tet, June 24, 2013. Equation (16)
...
// caclulate finite granular strain, equation (6)
...
// calculate 1/(2V)*sum(dudx'*dudx*vL), for mixed finite strain, Bagi's strian, equation (1)
matrix assembly::getHigherStrain() const{
	...
	for(std::vector<cell*>::const_iterator iter=cellVec_init.begin(); iter!=cellVec_init.end(); iter++){
		temp_volume = (*iter)->getInitialCellVolume();
		temp_dudx = getdudx(**iter);	// calculate equation (1)
...
// caclulate finite granular strain, equation (11)
matrix assembly::getEulerianStrain(){
	...
	for(std::vector<cell*>::const_iterator iter=cellVec.begin(); iter!=cellVec.end(); iter++){
		...
		temp_dudx = getdudx_curr(**iter);	// equation (12)
...
// caclulate average spatial velocity gradient tensor, June 24, 2013. Equation (17)
matrix assembly::getAverage_dvdx() const{
	...
	REAL temp_volume;
	for(std::vector<cell*>::const_iterator iter=cellVec.begin(); iter!=cellVec.end(); iter++){
		temp_volume = getCellVolume(**iter);	// volume, by equation (34)
		temp_dvdx = getdvdx_curr(**iter);	// calculate equation (16)
...
// caclulate finite granular strain, Higher Order Term in Eulerian strain tensor
...
void assembly::triaxialCompression(int   total_steps,  
			REAL displ,
			REAL sigma_a,
			int   first_snapshot,
			int   snapshots, 
			...
			const char* debugfile) 
{
    ...
    progressinf << setw(OWID) << "iteration"
		...
		<< setw(OWID) << "sigma1_1"
	        ...
	        << setw(OWID) << "t_step"
	        << setw(OWID) << "time" 
	        << setw(OWID) << "epsilon_11"	// for finite strain, Item (5) in page 157 in paper "http://dx.doi.org/10.1016/j.ijsolstr.2015.04.012"
	        << setw(OWID) << "epsilon_12"
	        << setw(OWID) << "epsilon_13"
	        << setw(OWID) << "epsilon_21"
	        << setw(OWID) << "epsilon_22"
	        ...
	        << setw(OWID) << "epsilon_32"
	        << setw(OWID) << "epsilon_33"
	        << setw(OWID) << "epsilon_11"	// for mixed finite strain, Item (4) in page 157 in paper "http://dx.doi.org/10.1016/j.ijsolstr.2015.04.012" 
	        << setw(OWID) << "epsilon_12"
	        << setw(OWID) << "epsilon_13"
	        << setw(OWID) << "epsilon_21"
	        << setw(OWID) << "epsilon_22"
	        ...
	        << setw(OWID) << "epsilon_32"
	        << setw(OWID) << "epsilon_33"
	        << setw(OWID) << "epsilon_11"	// for eulerian finite strain. Item (6) in page 157 in paper "http://dx.doi.org/10.1016/j.ijsolstr.2015.04.012"
	        << setw(OWID) << "epsilon_12"
	        << setw(OWID) << "epsilon_13"
	        << setw(OWID) << "epsilon_21"
	        << setw(OWID) << "epsilon_22"
		...
		<< setw(OWID) << "epsilon_w"	// for logranthmic strain
	        ...
	        << setw(OWID) << "epsilon_h"
	        << setw(OWID) << "epsilon_v" 
	        << setw(OWID) << "dudx_11"	// for average_dudx_Bagi, used to test quadratic terms, equation (1) in paper "http://dx.doi.org/10.1016/j.ijsolstr.2015.04.012"
	        << setw(OWID) << "dudx_12"
	        << setw(OWID) << "dudx_13"
	        << setw(OWID) << "dudx_21"
	        << setw(OWID) << "dudx_22"
	        ...
	        << setw(OWID) << "dudx_32"
	        << setw(OWID) << "dudx_33" 
	        << setw(OWID) << "dudx_11"	// for average_dudx_Lagrangian, used to test quadratic terms. Equation (7)
	        << setw(OWID) << "dudx_12"
	        << setw(OWID) << "dudx_13"
	        << setw(OWID) << "dudx_21"
	        << setw(OWID) << "dudx_22"
	        ...
	        << setw(OWID) << "dudx_32"
	        << setw(OWID) << "dudx_33" 
	        << setw(OWID) << "dudx_11"	// for average_dudx_Eulerian, used to test quadratic terms. Equation (12)
	        << setw(OWID) << "dudx_12"
	        << setw(OWID) << "dudx_13"
	        << setw(OWID) << "dudx_21"
	        << setw(OWID) << "dudx_22"
	        ...
	        << setw(OWID) << "dudx_32"
	        << setw(OWID) << "dudx_33" 	        
		<< setw(OWID) << "dvdx_11"	// for spatial velocity gradient tensor, deformation rate tensor. Equation (17)
	        << setw(OWID) << "dvdx_12"
	        << setw(OWID) << "dvdx_13"
	        << setw(OWID) << "dvdx_21"
	        << setw(OWID) << "dvdx_22"
...
//printParticle("fractured_particles");
    ...
    REAL bottom_init = getApt(6).getz();
    REAL top_init = getApt(5).getz();
    REAL dispCompressRate = displ/double(total_steps);
    do
    {
	// 1. create possible boundary particles and contacts between particles
	findContact();
...
//	granularStress = getGranularStress();	// calculate granular stress
	...
	// displacement control
	minctl[0].tran=vec(0,0,-dispCompressRate);
	minctl[1].tran=vec(0,0, dispCompressRate);	// move bottom boundary up

	top_disp += dispCompressRate;
	bot_disp += dispCompressRate;
...
//	    }
	    ...
	    // calculate spatial velocity gradient tensor
	    spatial_dvdx.clear();
	    spatial_dvdx = getAverage_dvdx();	// calculate equation (17)
	    curr_dvdx.clear();
	    curr_dvdx = spatial_dvdx;
	
	    curr_rate = 0.5*(curr_dvdx+curr_dvdx.getTrans());	// current spatial deformation rate, equation (17)

	    curr_strain_rate = prev_strain_rate+(curr_rate-curr_dvdx.getTrans()*prev_strain_rate-prev_strain_rate*curr_dvdx)*TIMESTEP*interval; // equation (19)
