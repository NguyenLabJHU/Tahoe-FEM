...
//      updateGridMaxZ();	// do not update grid in DEM-SPH coupling model
  ...
  void Assembly::generateSPHParticle2D(){
	...
	int num_layers = dem::Parameter::getSingleton().parameter["numLayers"];
 	REAL L = dem::Parameter::getSingleton().parameter["waterLength"];
	REAL nSPHPoint = dem::Parameter::getSingleton().parameter["nSPHPoint"];
...
//			sph::SPHParticle* tmp_pt = new sph::SPHParticle(SPHmass, SPHInitialDensity, tmp_x, 0, tmp_z, 1);	// 1 is free SPH particle
  ...
  // here the free particles, ghost particles and boundary particles will all stored in the same vector
  // unlike the implementation in serial version.
  void Assembly::generateSPHParticle3D(){
...
//			sph::SPHParticle* tmp_pt = new sph::SPHParticle(SPHmass, SPHInitialDensity, tmp_x, 0, tmp_z, 1);	// 1 is free SPH particle
  ...
  void Assembly::findSPHParticleInRectangle(const Rectangle &container,
  ...
  void Assembly::removeSPHParticleOutRectangle() {
    Vec  v1 = container.getMinCorner();
    Vec  v2 = container.getMaxCorner();
    REAL x1 = v1.getX();
    REAL y1 = v1.getY();
  ...
  void Assembly::scatterDEMSPHParticle() {
    // partition particles and send to each process
    if (mpiRank == 0) { // process 0
      int num_layers = dem::Parameter::getSingleton().parameter["numLayers"];
  ...
  void Assembly::commuSPHParticle() 
  {
    // determine container of each process
    Vec v1 = grid.getMinCorner();
    Vec v2 = grid.getMaxCorner();
  ...
  void Assembly::releaseRecvSPHParticle() {
    // release memory of received particles
  ...
  void Assembly::gatherSPHParticle() {
  ...
  void Assembly::releaseGatheredSPHParticle() {
    // clear allParticleVec, avoid long time memory footprint.
  ...
  void  Assembly::openSPHTecplot(std::ofstream &ofs, const char *str) {
    ofs.open(str);
  ...
  void Assembly::printSPHTecplot(std::ofstream &ofs, int iframe) {
...
//}
  ...
  void Assembly::printSPHParticle(const char *str) const{
    	std::ofstream ofs(str);
...
#pragma omp critical
			       ...
			       const char *boundaryFile)
  {
    ...
    void Assembly::initialSPHVelocity2D(){
	commuParticle();
...
//	checkNeighborCells2D();	// pass, May 22, 2015
	...
	dem::Vec tmp_vec = dem::Vec(0,0, -(dem::Parameter::getSingleton().parameter["gravAccel"])*(dem::Parameter::getSingleton().parameter["gravScale"]) );
...
//	}
    ...
    //// the momentum equilibrium equation and state equation are implemented as in Monaghan's paper (1994), simulate free surface flow using sph
    //// here the neighboring list of SPH particles is searched by the cells, vector< vector<sph::SPHParticle*> > SPHParticleCellVec;
    void Assembly::calculateSPHDensityDotVelocityDotLinkedList3D(){
