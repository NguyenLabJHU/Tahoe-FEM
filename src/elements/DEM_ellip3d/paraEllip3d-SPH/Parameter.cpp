...
namespace dem {
  ...
  void Parameter::readIn(const char *input) {
    ...
    case 1001: // proceed from preset state
      for (std::size_t i = 0; i < 2; ++i) {
	while (getline(ifs, line) ) if (line[0] != '#' && line.compare("") != 0 ) break;
	ssline.clear(); ssline.str(line);
	ssline >> str >> str2;
	...
	parameter[str] = val;
      ...
      parameter["soundSpeed"] = sqrt(200*parameter["gravAccel"]*parameter["gravScale"]*parameter["waterLength"]);
      parameter["P0"] = parameter["soundSpeed"]*parameter["soundSpeed"]*parameter["SPHInitialDensity"]/parameter["gamma"];
      parameter["P0"] = 1.013e5;
      break;

    case 1002: // proceed from preset state
      for (std::size_t i = 0; i < 2; ++i) {
	while (getline(ifs, line) ) if (line[0] != '#' && line.compare("") != 0 ) break;
	ssline.clear(); ssline.str(line);
	ssline >> str >> str2;
