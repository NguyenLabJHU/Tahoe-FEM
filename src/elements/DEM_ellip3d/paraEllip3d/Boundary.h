...
template<typename T> class Boundary {
 ...
 public:
  ...
  virtual void findBdryContact(std::vector<T*>& ptcls) {}
...
template<typename T> class plnBoundary : public Boundary<T> {
 ...
 private:
  ...
  template<class Archive>
    void serialize(Archive & ar, const unsigned int version) {
    ar & boost::serialization::base_object<Boundary<T> >(*this); // Boundary<T>
    ar & normal;
    ar & tangt;
    ar & moment;
    ar & possBdryParticle;
 ...
 public:
  ...
  void findBdryContact(std::vector<T*>& ptcls);
  Vec getApt() const;
  Vec getDirc() const;
  plnBoundary<T>* getBdry(int bdryid) const { return this; }
...
template<typename T>
void plnBoundary<T>::findBdryContact(std::vector<T*>& ptcls) {
...
template<typename T> class cylBoundary : public Boundary<T> {
 public:
  Vec normal; 
...
template<typename T>
void cylBoundary<T>::findBdryContact(std::vector<T*> &ptcls) {
  typename std::vector<T*>::iterator it;
  std::vector<BdryCoef>::iterator bt;
  bool next;
  possBdryParticle.clear();
