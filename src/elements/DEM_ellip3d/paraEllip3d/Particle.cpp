...
namespace dem {
  ...
  void Particle::init() {
    // 0-uniform distribution of orientations generated by spherical coordinates on unit sphere using equal-area projection.
    // 1-uniform distribution of orientations generated by Euler angles. 
    // 2-non-uniform distribution of orientations generated by Euler angles.
    int method = 1; // default 1
	...
	z = cos(theta);
	v2= Vec(x,y,z);
      } while (vfabs(v1 % v2) < EPS); // two vectors in parallel
      Vec v3 = v2 - (v2*v1) * v1;
      v3 = v3 / vfabs(v3); // normalize
      l2 = v3.getX(); m2 = v3.getY(); n2 = v3.getZ();

      Vec v4 = v1 % v3;
      l3 = v4.getX(); m3 = v4.getY(); n3 = v4.getZ();

    } else if (method == 1) {
      // uniform orientation by Euler angles 
      // according to Rotation, Reflection, and Frame Changes-Brannon.pdf:
      // angle1: \theta
      // angle2: \varphi
      // angle3: \psi
      ...
      // non-uniform orientations by Euler angles
      // according to Chinese Mathmatical Handbook.pdf:
      // angle1: \theta
      // angle2: \psi
      // angle3: \varphi
      REAL angle1, angle2, angle3; // angle1=[0,Pi], angle2=[0,2*Pi), angle3=[0,2*Pi]
