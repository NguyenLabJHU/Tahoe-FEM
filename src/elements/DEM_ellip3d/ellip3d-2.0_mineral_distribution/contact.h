...
void contact<T>::contactForce(bool &exceed){
	...
	REAL young1 = p1->young;
	REAL poisson1 = p1->poisson;
	REAL young2 = p2->young;
	REAL poisson2 = p2->poisson;

	R0=radius1*radius2/(radius1+radius2);
//	E0=0.5*YOUNG/(1-POISSON*POISSON);
	E0=young1*young2/((1-poisson1*poisson1)*young2 + (1-poisson2*poisson2)*young1);	// see equation (3.26) in Beichuan's thesis
	...
	p1->addMoment( ( (point1+point2)*0.5-p1->getCurrPosition() ) *   NormalForce );
	p2->addMoment( ( (point1+point2)*0.5-p2->getCurrPosition() ) * (-NormalForce) );	
	...
	*/
	...
	p1->addMoment( ( (point1+point2)*0.5-p1->getCurrPosition() ) * (-CntDampingForce) );
	p2->addMoment( ( (point1+point2)*0.5-p2->getCurrPosition() ) * CntDampingForce );
	...
	if (FRICTION != 0) {
	    ...
	    // follow equation (2) in paper by Regueiro and Zheng, modulus_distribution_DEM.pdf
	    REAL mu_alpha = young1/(2*(1+poisson1));
	    REAL mu_beta  = young2/(2*(1+poisson2));

	    REAL mu1 = mu_alpha*mu_beta/((2-poisson1)*mu_beta + (2-poisson2)*mu_alpha);
	    ks = 8*mu1*contact_radius;	// if poisson1==poisson2, then the ks here is the ks=4*G0*contact_radius/(2-POISSON)
...
//	    ks = 4*G0*contact_radius/(2-POISSON);
	    ...
	    */
	    ...
	    p1->addMoment( ((point1+point2)*0.5-p1->getCurrPosition())*TgtForce);
	    p2->addMoment(-((point1+point2)*0.5-p2->getCurrPosition())*TgtForce);
	}
