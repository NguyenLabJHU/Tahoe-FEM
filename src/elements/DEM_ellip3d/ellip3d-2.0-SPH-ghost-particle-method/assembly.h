...
class assembly{
 ...
 public:
  ...
  ~assembly(){
    ...
    std::vector<sph::SPHParticle*>::iterator spht;
    ...
    for(ct = cellVec_init.begin(); ct != cellVec_init.end(); ++ct)
      delete (*ct);
    for(spht = SPHParticleVec.begin(); spht != SPHParticleVec.end(); ++spht)
      delete (*spht);
    for(spht = SPHBoundaryParticleVec.begin(); spht != SPHBoundaryParticleVec.end(); ++spht)
      delete (*spht);
  ...
  REAL getCurrCellVolume() const;	// return the summed volumes of all the cells at current step  
...
//		       REAL sampleVolumeRatio,   // Modified by Boning Zhang
  ...
  void deposit_Compare(int   freetype,
		       int   total_steps,  
		       int   snapshots,
		       int   interval,
		       REAL  rFloHeight,
...
// apply the same rotation tensor to each particle directly such that we can make sure that the rotations of all particles are the same pure rotation. Test finite granular strain. June 10, 2013
  ...
  // --------------------- SPH part ------------------------------
    ...
    void readSPHData(const char *);	// should be called after readParticle in DEM

//    void writeMesh(const char *);
//    void writeMeshCheckVolume(const char *);
    void writeSPHParticleTecplot(std::ofstream &, const int);
    void printSPHVelocity(const char*);
    void printSPHVelocityPressure(const char*);
    void printSPHFreeParticle(const char* str);

    void initialSPHVelocity();	// initialization of SPH velocity based on equation (4.3) in http://www.artcompsci.org/vol_1/v1_web/node34.html
    void calculateSPHDensityDotVelocityDot();	// calculate densityDot and velocityDot based on equation (6) and equation (22)
    void calculateSPHDensityDotVelocityDotPeriodic(REAL );	// apply periodic boundary conditions in x,y also, http://en.wikipedia.org/wiki/Periodic_boundary_conditions
    void calculateSPHDensityDotVelocityDotLinkedList2D();	// calculate densityDot and velocityDot based on equation (6) and equation (22), by searching linked list
    void divideSPHDomain2D();	// devide the SPH domain into different cells which contain SPH particles within this cell
    void divideSPHDomain3D();	// devide the SPH domain into different cells which contain SPH particles within this cell
    void checkDivision();	// print out the particles in different cells in different files
    void checkNeighborCells2D();
    void updateSPH();
    void updateSPHLeapFrogVelocity();
    void updateSPHLeapFrogPositionDensity();
    void initialSPHLeapFrogVelocity();
    REAL calculateSPHKineticEnergy();
    REAL getWaterHeight();
    REAL getWaterWidth();
    ...
    void CouetteFlow();	// to test the sph implementation, refer to Morris's "Modeling low reynolds number incompressible flows using sph"
    void PoiseuilleFlow();
    void Cylinder2DProblem(int nsteps,              // total_steps
			   ...
			   const char* iniptclfile,   
		       	   ...
		       	   const char* contactfile,
		       	   const char* progressfile);
    void BurstingDam2D(int nsteps,              // total_steps
	       		   int snapshots,           // number of snapshots   
	         	   int interval,
			   const char* iniptclfile,
			   const char* inibdryfile,
		       	   ...
		       	   const char* contactfile,
		       	   const char* progressfile);
    void BurstingDam2DAfterEquilibrium(int nsteps,              // total_steps
	       			     int snapshots,           // number of snapshots   
	          		     int interval,
				     const char* iniptclfile,   
				     const char* inibdryfile,
				     const char* iniSPHptcl,
		       		     ...
		       		     const char* contactfile,
		       		     const char* progressfile);
    void Connectivity2DAfterEquilibrium(int nsteps,              // total_steps
	       			     int snapshots,           // number of snapshots   
	          		     int interval,
				     const char* iniptclfile,   
				     const char* inibdryfile,
				     const char* iniSPHptcl,
		       		     ...
		       		     const char* contactfile,
		       		     const char* progressfile);
    void BurstingDam3D(int nsteps,              // total_steps
	       		   int snapshots,           // number of snapshots   
	         	   int interval,
			   const char* iniptclfile,
			   const char* inibdryfile,
 ...
 private:
  ...
  // ------------------------------ SPH variables ----------------------------
    int nSPHPoint;	// number of SPH points
//    int nsteps;		// number of total steps
//    int rampStep; 	// steps for the applied boundary to reach maximum
//    int snapshots;	// print interval
    REAL SPHxmin, SPHxmax;	// the most left and right x dimensions, 
    REAL SPHymin, SPHymax;	// used to partition the computational domain
    REAL SPHzmin, SPHzmax;

    REAL height_z, length_x;	// for the 2D bursting dam
    ...
    REAL p1;	// for the Lennard-Jones boundary forces 
    REAL p2;

    std::vector<sph::SPHParticle*> SPHParticleVec;	// coordinates of all the particles in this domain
    std::vector<sph::SPHParticle*> SPHBoundaryParticleVec;

    // linked-list neighbor lists search for SPH
    ...
    int numCell;	// number of SPH cells
    int Nx;
    int Ny;
    int Nz;	// number of SPH cells in z direction

    // variables for openmp
    int numFreeSPH;	// number of free sph particles
    int numBoundarySPH;	// number of boundary sph particles

    // parameters for sph kernel
    REAL one_devide_h;
    REAL factor_kernel;
    REAL factor_kernel_gradient;

//    // parameters of the infinite height cylinder boundary, the height is in z direction
//    REAL center_x;
//    REAL center_y;	// the x,y position of the center of the cylinder
