...
/*
   ...
   void gauss1D(const int nintElem, dem::Matrix& s, dem::Matrix& w ) {
	...
	void shp3d(const REAL xi, const REAL eta, const REAL zeta, dem::Matrix& xl, dem::Matrix& shp, REAL& xsj) {
...
/*
    ...
    REAL kernelFunction(const dem::vec& a, const dem::vec& b, REAL h){
	REAL rab = dem::vfabs(a-b);
	REAL s = rab/h;
	REAL item1_5 = (2-s)*(2-s)*(2-s);	// (3-s)^5
	REAL item2_5 = (1-s)*(1-s)*(1-s); 	// (2-s)^5
//	REAL factor = 1.0/(120.0*dem::PI*h*h*h);	// 3D quintic kernel factor
	REAL factor = 15.0/(14.0*6.0*dem::PI*h*h);		// 2D quintic kernel factor
	if(s<1){
	    return factor*(item1_5-4*item2_5);
	}
	else if(s<2){
	    return factor*(item1_5);
	}
	else{
	    return 0.0;
	}
    ...
    REAL kernelFunction(REAL s, REAL h){
	REAL item1_5 = (2-s)*(2-s)*(2-s);	// (3-s)^5
	REAL item2_5 = (1-s)*(1-s)*(1-s); 	// (2-s)^5
//	REAL factor = 1.0/(120.0*dem::PI*h*h*h);	// 3D quintic kernel factor
	REAL factor = 15.0/(14.0*6.0*dem::PI*h*h);	// 2D quintic kernel factor
    ...
    dem::vec gradientKernelFunction(const dem::vec& a, const dem::vec& b, REAL h){
	REAL rab = dem::vfabs(a-b);
	REAL s = rab/h;
	REAL item1_4 = (2-s)*(2-s);	// (3-s)^4
	REAL item2_4 = (1-s)*(1-s); // (2-s)^4
//	REAL factor = 1.0/(120.0*dem::PI*h*h**h*h);	// 3D quintic kernel factor
//	REAL factor = 1.0/(120.0*h*h);	// 1D quintic kernel factor
	REAL factor = 15.0/(14.0*6.0*dem::PI*h*h*h);	// 2D quintic kernel factor
	if(s<1){
	    return factor*(-3*item1_4+12*item2_4)*(a-b)/rab;
	}
	else if(s<2){
	    return factor*(-3*item1_4)*(a-b)/rab;
	}
	else{
	    return dem::vec(0.0);
	}
    ...
    REAL partialKernelFunction(const dem::vec& a, const dem::vec& b, REAL h){
	REAL rab = dem::vfabs(a-b);
	REAL s = rab/h;
	REAL item1_4 = (2-s)*(2-s);	// (3-s)^4
	REAL item2_4 = (1-s)*(1-s); // (2-s)^4
...
//	REAL factor = 1.0/(120.0*h*h);	// 1D quintic kernel factor
	REAL factor = 15.0/(14.0*6.0*dem::PI*h*h*h);	// 2D quintic kernel factor
	if(s<1){
	    return factor*(-3*item1_4+12*item2_4);
	}
	else if(s<2){
	    return factor*(-3*item1_4);
	}
	else{
	    return 0.0;
	}
