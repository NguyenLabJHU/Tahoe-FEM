...
// this is for generating 2D particles
curr_direction_a = vec(0, dem::PI*0.5, dem::PI*0.5);
curr_direction_b = vec(dem::PI*0.5, 0, dem::PI*0.5);
curr_direction_c = vec(dem::PI*0.5, dem::PI*0.5, 0);
...
REAL particle::getMaxRadius() const{
    REAL tmp;
    tmp = a;
    if(tmp<b) tmp=b;
    if(tmp<c) tmp=c;
...
//	curr_velocity.setz(-PILE_RATE);
	curr_velocity.setx(-PILE_RATE);	
	curr_velocity.sety(0);
	curr_velocity.setz(0);
	curr_position = prev_position + curr_velocity*TIMESTEP;
    }
...
vec particle::cylinderRBForce(int bdry_id, const cylinder& S, int side){
	...
	for(std::vector<sph::SPHParticle*>::iterator pt=SPHGhostParticleVec.begin(); pt!=SPHGhostParticleVec.end(); pt++){
	    sph_local = (*pt)->getLocalPosition();	// not correct!!!!!!!!!!!!, since localVec is changed
	    sph_curr = globalVec(sph_local)+curr_position;
sph_curr.sety(0);
