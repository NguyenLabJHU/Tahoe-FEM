...
namespace dem {
  ...
  void Parameter::readIn(const char *input) {
	...
	parameter[str] = val;
      ...
      parameter["lambda"] = parameter["periPoisson"]*parameter["periYoung"]/((1.0+parameter["periPoisson"])*(1.0-2.0*parameter["periPoisson"]));
      parameter["mu"] = parameter["periYoung"]/(2.0*(1.0+parameter["periPoisson"]));
      parameter["kBulk"] = parameter["periYoung"]/(3.0*(1.0-2.0*parameter["periPoisson"]));
      ...
      parameter["Aphi"] = 2*sqrt(6.0)*cos(parameter["phi"])/(3.0+parameter["beta"]*sin(parameter["phi"]));
      parameter["Bphi"] = 2*sqrt(6.0)*sin(parameter["phi"])/(3.0+parameter["beta"]*sin(parameter["phi"]));
      parameter["Apsi"] = 2*sqrt(6.0)*cos(parameter["psi"])/(3.0+parameter["beta"]*sin(parameter["psi"]));
      parameter["Bpsi"] = 2*sqrt(6.0)*sin(parameter["psi"])/(3.0+parameter["beta"]*sin(parameter["psi"]));

      break;

    case 3002: // couple with peridynamics clay, pull out DEM particles in peri-domain
      for (std::size_t i = 0; i < 2; ++i) {
	while (getline(ifs, line) ) if (line[0] != '#' && line.compare("") != 0 ) break;
	ssline.clear(); ssline.str(line);
	ssline >> str >> str2;
