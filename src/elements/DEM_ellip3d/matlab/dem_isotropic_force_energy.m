function dem_isotropic_force_energy(fileToRead1)
...
subplot(1,2,1)
[AX,H1,H2] = plotyy(epsilon_v, [avgNormal, avgShear], epsilon_v, avgPenetr);
grid on;
grid minor;
pbaspect(AX(1), [1 1 1]);
pbaspect(AX(2), [1 1 1]);
...
set(AX(1),'YColor','b');
set(AX(2),'YColor','r');
%line(epsilon_v, avgShear, 'Parent', AX(1), 'Color', 'k', 'LineWidth', 3);
...
lh=legend('avgNormal', 'avgShear', 'penetration', 'location', 'northwest');
set(lh, 'box', 'off');
...
forceH = plot(accruedTime, transEnergy, accruedTime, rotatEnergy, accruedTime, kinetEnergy, 'LineWidth', 3); 
grid on;
grid minor;
pbaspect([1 1 1])
title('Energy');
xlabel('Step');
ylabel('Energy (J)');
lh=legend('transEnergy', 'rotatEnergy', 'kinetEnergy', 'location', 'northeast');
set(lh, 'box', 'off');
%axis([0, 3.5e-4, ylim]);
%xlim([0, 4000]);
