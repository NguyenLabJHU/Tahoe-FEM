...
int main(int argc, char *argv[])
{
  if(argc < 3) {
    cout << endl 
	 << "-- Plot particle centroid tetrahedrons, filtered by distance gap (0.2 by default) --"<<endl
	 << "Usage:" << endl
	 << "1) process a single file:  plot_tetra_particle particle_file gap_ratio" << endl
	 << "   --example: plot_tetra_particle triaxial_particle_008 0.2" << endl
	 << "2) process multiple files: plot_tetra_particle particle_file_prefix first_suffix last_suffix suffix_increment gap_ratio" << endl
	 << "   --example: plot_tetra_particle triaxial_particle 1 100 5 0.2" << endl << endl;
    return -1;
  }	
  ...
  if(argc == 3) {
    ...
    last  = 1;
    incre = 2;
    gapRatio = atof(argv[2]);
  }
  else {
    first = atoi(argv[2]);
    last  = atoi(argv[3]);
    incre = atoi(argv[4]);
    gapRatio = atof(argv[5]);
  }
  ...
  for(int step=first; step<=last; step+=incre) {
    ...
    std::vector<Particle> particleVec(totalParticle);
    for(int k = 0; k < totalParticle; ++k) {
      ...
      // store particle info
      particleVec[k].coord = Vec(position_x, position_y, position_z);     
      particleVec[k].radius= radius_a;
    }
    ifs.close();
    ...
    for(int it=0; it!=totalTetra; it++) {
      ...
      Tetra tmpTetra(i, j, k, l, 
		     particleVec[i-1].coord, 
		     particleVec[j-1].coord, 
		     particleVec[k-1].coord, 
		     particleVec[l-1].coord, 
		     particleVec[i-1].radius, 
		     particleVec[j-1].radius, 
		     particleVec[k-1].radius, 
		     particleVec[l-1].radius);
      if (tmpTetra.isValid(gapRatio) && fabs(tmpTetra.getVolume()) > EPS) // only store non-coplanar tetrahedrons
...
VARLOCATION=([4,5,6,7,8,9,10]=CELLCENTERED), ZONETYPE=FETETRAHEDRON, STRANDID=4, SOLUTIONTIME=" << step << endl;
    ...
    // coordinates
    kut = 0;
    for (std::size_t i = 0; i < particleVec.size(); ++i) {
      ofs << std::setw(OWID) << particleVec[i].coord.getX();
    ...
    kut = 0;
    for (std::size_t i = 0; i < particleVec.size(); ++i) {
      ofs << std::setw(OWID) << particleVec[i].coord.getY();
      ++kut; if (kut >= valNum) {ofs << std::endl; kut = 0;}
    }
    ofs << std::endl;
    ...
    kut = 0;
    for (std::size_t i = 0; i < particleVec.size(); ++i) {
      ofs << std::setw(OWID) << particleVec[i].coord.getZ();
      ++kut; if (kut >= valNum) {ofs << std::endl; kut = 0;}
    }
    ofs << std::endl;
	...
	<< setw(OWID) << "z"
	<< endl;
    ofs2<< "ZONE N=" << totalParticle << ", E=" << tetraVec.size()  <<", DATAPACKING=POINT, ZONETYPE=FETETRAHEDRON, STRANDID=4, SOLUTIONTIME=" << step << endl;
    for (int i = 0; i < particleVec.size(); ++i)
      ofs2 << std::setw(OWID) << particleVec[i].coord.getX() << std::setw(OWID) << particleVec[i].coord.getY() << std::setw(OWID) << particleVec[i].coord.getZ() << std::endl;
    for (std::size_t i = 0; i < tetraVec.size(); ++i)
