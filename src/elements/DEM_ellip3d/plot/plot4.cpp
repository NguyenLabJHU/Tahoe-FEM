...
int main(int argc, char *argv[]) {
  ...
  int id, type, TotalNum, particleShape;
  ...
  for(n=first; n<=last; n+=incre) {
    ...
    ifs >> TotalNum >> particleShape;

    if (particleShape == 0) { // Ellipsoid
      long double a, b, c;
    ...
    } else if (particleShape == 1) { // PolyEllipsoid

      long double a_plus, a_minus, b_plus, b_minus, c_plus, c_minus;
