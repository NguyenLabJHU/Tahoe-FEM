    ...
    % Extended by Boning Zhang to construct equivalent poly-ellipsoids
    %===============================================================
    clc
    clear all;
    ...
    if connect == 8   %%%% 2D
      ...
      if connect == 6   %%%3D
        ...
        n_9=[];
        n_10=[];
        n_12=[];objects_principal_vectors
        n_15=[];
        n_17=[];
        n_18=[];
        n_19=[];
    ...
    surface_area=length(object_boundary);
    ...
    objects_surface_area=[objects_surface_area;length(object_boundary)];
    ...
    % move the global object_voxels_final to the global
    % tmp_object_centroids, added on June 09, 2014
    object_voxels_final(:,1) = object_voxels_final(:,1) - temp_objects_centroids(1);
    object_voxels_final(:,2) = object_voxels_final(:,2) - temp_objects_centroids(2);
    object_voxels_final(:,3) = object_voxels_final(:,3) - temp_objects_centroids(3);
    
    [v,score]=pca(object_voxels_final,'Economy',false);

	length_points=[];
	
	%length_points=v*object_voxels_final(1:length(object_voxels_final(:,1)),:)';
	%length_points=abs(length_points');
	length_points=score;

%     comment on June 09, 2014. This commented method is use local
%     length_points - local local_centroids, we can use another method,
%     namely use global object_voxels_final - global temp_objects_centriods
%     first and then localize it
...
%     length_centroid = length_centroid';
    ...
    locate_zero=find(temp_objects_lengths==0);
    temp_objects_lengths(locate_zero)=1;
    

	objects_lengths=[objects_lengths;temp_objects_lengths];

%     comment on June 09, 2014    
...
%     length_points(:,3) = length_points(:,3)-length_centroid(3);
    ...
    % get the number of voxel points, namely length of length_points
    num_voxel_points = length(length_points(:,1));
    ...
    num_i_p = 0;
    for i_p=1:num_voxel_points    % loop over all the voxel points
        x_p = length_points(i_p,1);
        y_p = length_points(i_p,2);
        z_p = length_points(i_p,3);
        
        if(mod(i_p, floor(num_voxel_points/20)) == 0) % 20 outputs 
            num_i_p = num_i_p + 1;
            disp('the number of iterval now is '); disp(num_i_p)
        end
        ...
        if(x_p>=0 && y_p>=0 && z_p>=0)  % 1st quadrant
            ...
            if temp_v>0 % means this voxel point is above the plane
                above_v1 = above_v1+1;
            elseif temp_v<0 % means this voxel point is below the plane
                below_v1 = below_v1+1;
            else     % means this voxel point is in the plane
                above_v1 = above_v1+0.5;
                below_v1 = below_v1+0.5;
            end
        
        elseif(x_p<=0 && y_p>=0 && z_p>=0)  % 2nd quadrant
        ...
        elseif(x_p<=0 && y_p<=0 && z_p>=0)  % 3rd quadrant
        ...
        elseif(x_p>=0 && y_p<=0 && z_p>=0)  % 4th quadrant
        ...
        elseif(x_p>=0 && y_p>=0 && z_p<=0)  % 5th quadrant
        ...
        elseif(x_p<=0 && y_p>=0 && z_p<=0)  % 6th quadrant
        ...
        elseif(x_p<=0 && y_p<=0 && z_p<=0)  % 7th quadrant
  ...
  while(1)
    ...
    for i_p=1:num_voxel_points    % loop over all the voxel points
        ...
        if(mod(i_p,floor(num_voxel_points/20)) == 0) % 20 outputs 
            num_i_p = num_i_p +1;
            disp('the number of interval now is '); disp(num_i_p)
        end
        ...
        else %(x_p>=0 && y_p<=0 && z_p<=0)  % 8th quadrant
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for i=1:length(coord_number)
       ...
       temp_center_center_vector=[fake_i_zone, coord_number(i), centroid_contact(:,1)-objects_centroids(i_zone,1),...
        ...
        %if size(contact_points_coordinats,1)>=2
            
        [coeff]=pca(contact_points_coordinats,'Economy',false);
