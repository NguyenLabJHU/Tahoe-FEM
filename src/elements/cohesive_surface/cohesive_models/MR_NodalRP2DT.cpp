/* $Id: MR_NodalRP2DT.cpp,v 1.1 2006-10-08 19:22:19 regueiro Exp $  */
...
/* traction vector given displacement jump vector */	
const dArrayT& MR_NodalRP2DT::Traction_penalty(const dArrayT& jump_u, ArrayT<double>& state, const dArrayT& sigma, bool qIntegrate)
{
	const char caller[] = "MR_NodalRP2DT::Traction_penalty";
...
// at which the zone initiates - handle this with the state flag ????????
	...
	else /* calculate tractions */
	{
		...
		dArrayT traction_last(2, state.Pointer(k_T_t));
		...
		if (qIntegrate && update_traction)
		{
			...
			//is this correct?
			if (!t_free) fTraction[0] = traction_last[0] 
				+ fConstraintStiffness*(jump_u[0] - state[k_up_t])/(*fArea);
			if (!n_free) fTraction[1] = traction_last[1] 
				+ fConstraintStiffness*(jump_u[1] - state[k_up_n])/(*fArea);
			...
			{
				if (kk > 500) {
					ExceptionT::GeneralFail("MR_NodalRP2DT::Traction_penalty","Too Many Iterations");
				}
...
/* potential stiffness */
const dMatrixT& MR_NodalRP2DT::Stiffness_penalty(const dArrayT& jump_u, const ArrayT<double>& state, 
	const dArrayT& sigma)
{
	const char caller[] = "MR_NodalRP2DT::Stiffness_penalty";
...
#pragma unused(sigma)
	...
	/* Sheng-Kai, you must modify this code for using the fConstraintStiffness */
