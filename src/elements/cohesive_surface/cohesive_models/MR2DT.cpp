/*$Id: MR2DT.cpp,v 1.27 2007-07-22 03:44:15 skyu Exp $*/
/* created by manzari*/
		...
		if (ff < 0.) {
			...
			while (ff > fTol_1 || normr > fTol_2) {
				...
				mr_ep_2d_out << setw(outputFileWidth) << "yield_f = " << ff
					...
					<< endl;
				mr_ep_2d_out << setw(outputFileWidth) << "T_t = " << Sig[0]
					<< setw(outputFileWidth) << "T_n = " << Sig[1]
					<< endl;
...
#pragma unused(jump_u)
	...
	*/
	...
	dArrayT qn(4), Sig(2), dQdSig(2), dfdq(4), qbar(4), dfdSig(2), H1(2), KEP1(2);
	...
	if (state[12] == 0.){
		...
		for (i = 0; i <= 1; ++i){
			H1[i] = dfdSig[0]*KEE(0,i) + dfdSig[1]*KEE(1,i);
		}
		// H1[0] = dfdSig[0]*KEE(0,0) + dfdSig[1]*KEE(1,0);
		// H1[1] = dfdSig[0]*KEE(0,1) + dfdSig[1]*KEE(1,1);
