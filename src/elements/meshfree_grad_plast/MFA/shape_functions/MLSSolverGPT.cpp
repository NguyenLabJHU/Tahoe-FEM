...
MLSSolverGPT::MLSSolverGPT(int nsd, int complete, MeshFreeT::WindowTypeT window_type, 
	...
	fDDDb(dSymMatrixT::NumValues(fNumSD)),// kyonten
	fDM(fNumSD),
	fDDM(dSymMatrixT::NumValues(fNumSD)),
	fDDDM(dSymMatrixT::NumValues(fNumSD)), // kyonten
	...
	fNSDvec(fNumSD) // kyonten (for DDDw)
{
	...
	{
		case 1:
			fBasis = new PolyBasis1DGPT(fComplete);
			break;
		case 2:
			fBasis = new PolyBasis2DGPT(fComplete);
			break;
		case 3:
			fBasis = new PolyBasis3DGPT(fComplete);
			break;
		default:
...
void MLSSolverGPT::ComputeDDDM(const dArrayT& volume) // kyonten (DDDM)
{
	int dim = fBasis->BasisDimension();
	const dArray2DT& basis = fBasis->P();
