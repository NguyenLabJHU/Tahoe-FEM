...
/* constructor */
MeshFreeSSSolidGPT::MeshFreeSSSolidGPT(const ElementSupportT& support, const FieldT& field):
	...
	fB_wrap(10, fB)
{
	...
	{
		cout << "\n MeshFreeSSSolidGPT::MeshFreeSSSolidGPT: no strain-displacement options\n" << endl;
		fStrainDispOpt = kStandardB;
	}

	/* check */
	if (AutoBorder() && ElementSupport().Size() > 1)
		ExceptionT::BadInputValue("MeshFreeSSSolidGPT::MeshFreeSSSolidGPT", "auto-border not support in parallel");
}
...
ExceptionT::GeneralFail("MeshFreeSSSolidGPT::Initialize", "out of date");
	...
	{	
		cout << "\n MeshFreeSSSolidGPT::Initialize: can only have 1 material in the group\n";
...
void MeshFreeSSSolidGPT::Equations(AutoArrayT<const iArray2DT*>& eq_1,
...
void MeshFreeSSSolidGPT::ConnectsU(AutoArrayT<const iArray2DT*>& connects_1,
...
/* write output */
void MeshFreeSSSolidGPT::WriteOutput(void)
{
	/* inherited */
...
* the last time step */
GlobalT::RelaxCodeT MeshFreeSSSolidGPT::RelaxSystem(void)
{
	/* inherited */
...
int MeshFreeSSSolidGPT::InterpolantDOFs(void) const
{
...
void MeshFreeSSSolidGPT::WeightNodalCost(iArrayT& weight) const
{
	/* inherited */
	WeightNodes(weight);
}
...
void MeshFreeSSSolidGPT::NodalDOFs(const iArrayT& nodes, dArray2DT& DOFs) const
{
	/* inherited */
...
void MeshFreeSSSolidGPT::InitStep(void)
{
	/* inherited */
...
void MeshFreeSSSolidGPT::CloseStep(void)
{
	/* inherited */
...
GlobalT::RelaxCodeT MeshFreeSSSolidGPT::ResetStep(void)
{
	/* inherited */
...
void MeshFreeSSSolidGPT::PrintControlData(ostream& out) const
{
	/* inherited */
...
void MeshFreeSSSolidGPT::SetShape(void)
{
	/* only support single list of integration cells for now */
	if (fConnectivities.Length() > 1) {
		cout << "\n MeshFreeSSSolidGPT::SetShape: multiple element blocks within an\n"
...
bool MeshFreeSSSolidGPT::NextElement(void)
{
...
void MeshFreeSSSolidGPT::ComputeOutput(const iArrayT& n_codes, dArray2DT& n_values,
...
void MeshFreeSSSolidGPT::WriteField(void)
{
