/* $Id: MeshFreeElementSupportGPT.cpp,v 1.1 2004-07-08 20:01:13 kyonten Exp $ */
/* created: paklein (11/12/1999) */
#include "MeshFreeElementSupportGPT.h"
...
/* constructor */
MeshFreeElementSupportGPT::MeshFreeElementSupportGPT(ifstreamT& in):
	fMFShapes(NULL),
	fNodalShapes(NULL),
	fLocGroup(kHeadRoom),
	fNumElemenNodes(0),
...
/* accessors */
MeshFreeSupportT& MeshFreeElementSupportGPT::MeshFreeSupport(void) const
{
...
void MeshFreeElementSupportGPT::PrintControlData(ostream& out) const
{
	/* echo */
...
/* initialization */
void MeshFreeElementSupportGPT::InitSupport(ifstreamT& in, ostream& out,
...
int MeshFreeElementSupportGPT::SetElementNodes(int element)
{
...
void MeshFreeElementSupportGPT::SetNodalField(const dArray2DT& dof)
{
...
void MeshFreeElementSupportGPT::GetNodalField(const dArray2DT& dof,
...
void MeshFreeElementSupportGPT::FreeNodalField(void)
{
	/* unset flag */
	fFieldSet = false;
...
int MeshFreeElementSupportGPT::MarkActiveCells(AutoArrayT<ElementCardT>& elem_cards)
{
	/* loop over cells */
	int active_count = 0;
...
void MeshFreeElementSupportGPT::TraceNode(ostream& out, int node, const ElementBaseT& element_group)
{
	int d_width = out.precision() + kDoubleExtra;
	out << "\n MeshFreeElementSupportGPT::TraceNode: " << node + 1 << endl;
...
/* weight nodes */
void MeshFreeElementSupportGPT::WeightNodes(iArrayT& weight) const
{
...
/* class specific data */
void MeshFreeElementSupportGPT::EchoNodesData(ifstreamT& in, ostream& out, int max_node_num,
	ModelManagerT* model)
{
	...
	if (fOffGridNodes.Length() > 0) // empty sets
	  {
	    ...
	    if (fOffGridNodes.Min() < 0 || fOffGridNodes.Max() >= max_node_num)
	      {
		cout << "\n MeshFreeElementSupportGPT::EchoNodesData: off grid EFG node out of range" << endl;
	...
	if (fFENodes.Length() > 0) // empty sets
	  {
	    ...
	    if (fFENodes.Min() < 0 || fFENodes.Max() >= max_node_num)
	      {
		cout << "\n MeshFreeElementSupportGPT::EchoNodesData: interpolant node out of range" << endl;
		throw ExceptionT::kBadInputValue;
	      }
	  }
	...
	if (fEFGNodes.Length() > 0) // empty sets
	  {
	    ...
	    if (fEFGNodes.Min() < 0 || fEFGNodes.Max() > max_node_num)
	      {
		cout << "\n MeshFreeElementSupportGPT::EchoNodesData: set EFG node out of range" << endl;
		throw ExceptionT::kBadInputValue;
	      }
	  }
}
...
void MeshFreeElementSupportGPT::SetAllFENodes(const iArrayT& fe_nodes)
{
