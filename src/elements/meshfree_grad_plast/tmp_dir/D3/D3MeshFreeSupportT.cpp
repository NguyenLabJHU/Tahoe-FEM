/* $Id: D3MeshFreeSupportT.cpp,v 1.1 2004-09-28 23:13:59 raregue Exp $ */
	...
	dArray2DT& Dphi, dArray2DT& DDphi, dArray2DT& DDDphi)
{
	...
	if (fStoreShape)
	{
		...
		//DDDphi.Set(?nst, nnd, fnDDDPhiData(tag));
	}
	else
	{
		/* set shallow data */
	...
	dArray2DT& phi, ArrayT<dArray2DT>& Dphi, ArrayT<dArray2DT>& DDphi, ArrayT<dArray2DT>& DDDphi)
{
	...
	if (fStoreShape)
	{
		...
		for (int i = 0; i < nip; i++)
		{
			...
			/* 3rd derivatives */
			//DDDphi[i].Set(nst?, nnd, DDDphi_ptr);
...
void D3MeshFreeSupportT::InitNodalShapeData(void)
{
	...
	//fnDDDPhiData.Configure(fnNeighborCount, ?nst);	
}
...
void D3MeshFreeSupportT::InitElementShapeData(void)
{
	...
	//feDDDPhiData.Configure(feNeighborCount, nip*?nst);
}
