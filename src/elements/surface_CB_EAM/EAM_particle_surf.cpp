...
/* constructor */
EAM_particle_surf::EAM_particle_surf(CBLatticeT& lattice, const StringT& param_file): 
	fLattice(lattice),
	fEAMProperty(NULL),
	fPairEnergy(NULL),
	fPairForce(NULL),
...
/* Destructor */
EAM_particle_surf::~EAM_particle_surf(void) {
	delete fEAMProperty;
}

/* set "glue" functions and dimension work space */
void EAM_particle_surf::Initialize(int nsd, int numbonds)
{
...
*/
double EAM_particle_surf::ComputeUnitEnergy(void)
{
...
*/
void EAM_particle_surf::ComputeUnitStress(dSymMatrixT& stress)
{
	const dArrayT& r = fLattice.DeformedLengths();
	const iArrayT& counts = fLattice.BondCounts();
...
*/
void EAM_particle_surf::ComputeUnitModuli(dMatrixT& moduli)
{
...
*/
double EAM_particle_surf::TotalElectronDensity(void)
{
	const dArrayT& r = fLattice.DeformedLengths();
	const iArrayT& counts = fLattice.BondCounts();
...
double EAM_particle_surf::ReturnEmbeddingForce(double rho)
{
...
* ONLY.
*/
void EAM_particle_surf::FormMixedDerivatives(double rho)
{
	const dArrayT& r = fLattice.DeformedLengths();
	const iArrayT& counts = fLattice.BondCounts();
...
*/
void EAM_particle_surf::FormSingleBondContribution(double rho, dMatrixT& moduli)
{
	const dArrayT& r = fLattice.DeformedLengths();
	const iArrayT& counts = fLattice.BondCounts();
...
void EAM_particle_surf::FormMixedBondContribution(double rho, dMatrixT& moduli)
{
	/* batch fetch */
