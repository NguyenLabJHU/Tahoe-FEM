...
/*    fH1_8=0.0;
  ...
  //  dMatrixT LST;//Lagrangian strain tensor used in some functions to get rid of long name
    ...
    fdevSPK_tr=0.0;
     ...
     /* Matrice from the second bal. Eqn. */
     fKphiu_IIJp_1=0.0;
     fKphiu_IIJp_2=0.0;
     fKphiu_IIJp_3=0.0;
     fKphiu_IIJp_4=0.0;
...
//	    				fRight_Elastic_Cauchy_Green_tensor = fIdentity_matrix;
	    			...
	    			fS_SIGMA.SetToScaled(Temp_inv*fMaterial_Params[kTau],fIdentity_matrix);
	    			// 2sigmaE
	    			fTemp_matrix_nsd_x_nsd.SetToScaled(2*fMaterial_Params[kSigma_const],Elastic_LagrangianStn);
	    			fS_SIGMA+=fTemp_matrix_nsd_x_nsd;
	    			//(eta-Tau)trEpsilon.1
	    			Temp_inv=Elastic_MicroStnTensor.Trace();
	    			...
	    			scale =(1/fConst)*Trace_const*scale_const*fMaterial_Params[kNu]*Jp;
	    			fTemp_matrix_nudof_x_nudof *= scale;
	    			fKu_IJp_6 += fTemp_matrix_nudof_x_nudof;

	    		 	fTemp_matrix_nudof_x_nchidof.MultATBC(fShapeDisplGrad,IJp_7,NCHI);
	    			scale =(1/fConst)*Trace_const*scale_const*fMaterial_Params[kEta]*dFYdScol1*Jp;
	    			fTemp_matrix_nudof_x_nchidof *= scale;
	    			...
	    			scale=-1*(1/fConst)*(fMaterial_Params[kLambda]+fMaterial_Params[kTau])
	    			...
	    			*fMaterial_Params[kKappa]*scale_const*Jp;
	    			...
	    			scale =-1*(1/fConst)*(fMaterial_Params[kMu]+fMaterial_Params[kSigma_const])*fMaterial_Params[kNu]*scale_const*Jp;
	    			fTemp_matrix_nudof_x_nudof *= scale;
	    			...
	    			scale =-1*(1/fConst)*fMaterial_Params[kEta]*trfN1*fMaterial_Params[kNu]*scale_const*Jp;
	    			fTemp_matrix_nudof_x_nudof *= scale;
	    			...
	    			*scale_const*Jp;
	    			...
	    			scale =-1*(1/fConst)*(fMaterial_Params[kLambda]+fMaterial_Params[kTau])*trfA1*fMaterial_Params[kEta]*dFYdScol1*scale_const*Jp;
	    			fTemp_matrix_nudof_x_nchidof *= scale;
	    			...
	    			scale =-1*(1/fConst)*(fMaterial_Params[kLambda]+fMaterial_Params[kTau])*trfA1*fMaterial_Params[kNu]*scale_const*Jp;
	    			fTemp_matrix_nudof_x_nchidof *= scale;
	    			fKuphi_I3p_42 += fTemp_matrix_nudof_x_nchidof;

	    			fTemp_matrix_nudof_x_nchidof.MultATBC(fShapeDisplGrad,I3p_43,NCHI);
	    			scale =-1*(1/fConst)*(fMaterial_Params[kMu]+fMaterial_Params[kSigma_const])*fMaterial_Params[kEta]*dFYdScol1*scale_const*Jp;
	    			fTemp_matrix_nudof_x_nchidof *= scale;
	    			...
	    			scale =-1*(1/fConst)*fMaterial_Params[kEta]*fMaterial_Params[kEta]*dFYdScol1*trfN1*scale_const*Jp;
	    			fTemp_matrix_nudof_x_nchidof *= scale;
	    			fKuphi_I3p_46 += fTemp_matrix_nudof_x_nchidof;

	    			fTemp_matrix_nudof_x_nchidof.MultATBC(fShapeDisplGrad,I3p_47,NCHI);
	    			scale =-1*(1/fConst)*fMaterial_Params[kEta]*fMaterial_Params[kKappa]*trfN1*scale_const*Jp;
	    			fTemp_matrix_nudof_x_nchidof *= scale;
	    			fKuphi_I3p_47 += fTemp_matrix_nudof_x_nchidof;

	    			fTemp_matrix_nudof_x_nchidof.MultATBC(fShapeDisplGrad,I3p_48,NCHI);
	    			scale =-1*(1/fConst)*fMaterial_Params[kEta]*fMaterial_Params[kNu]*trfN1*scale_const*Jp;
	    			fTemp_matrix_nudof_x_nchidof *= scale;
	    			...
	    			scale =-1*(1/fConst)*fMaterial_Params[kLambda]*dFYdScol1*scale_const*Jp;
	    			fTemp_matrix_nudof_x_nudof *= scale;
	    			fKu_I4p_2 += fTemp_matrix_nudof_x_nudof;

	    			fTemp_matrix_nudof_x_nudof.MultATBC(fShapeDisplGrad,I4p_3,fShapeDisplGrad);
	    			scale =-1*(1/fConst)*fMaterial_Params[kMu]*scale_const*Jp;
	    			fTemp_matrix_nudof_x_nudof *= scale;
	    			...
	    			scale = (1/fConst)*Trace_const*fMaterial_Params[kNu]*scale_const*Jp;
	    			fTemp_matrix_nchidof_x_nchidof *= scale;
	    			...
	    			scale =(1/fConst)*(fMaterial_Params[kLambda]+fMaterial_Params[kTau])*dFYdScol1*scale_const*Jp;
	    			fTemp_matrix_nchidof_x_nudof *= scale;
	    			fKphiu_II2p_2 += fTemp_matrix_nchidof_x_nudof;

	    			fTemp_matrix_nchidof_x_nudof.MultATBC(NCHI,II2p_3,fShapeDisplGrad);
	    			scale =(1/fConst)*(fMaterial_Params[kMu]+fMaterial_Params[kSigma_const])*scale_const*Jp;
	    			fTemp_matrix_nchidof_x_nudof *= scale;
	    			...
	    			*fMaterial_Params[kNu];
	    			...
	    			scale =-1*scale_const*Jp*(1/fConst)*(fMaterial_Params[kNu]-fMaterial_Params[kSigma_const])
	    			*(fMaterial_Params[kLambda]+fMaterial_Params[kTau])*dFYdScol1;
	    			fTemp_matrix_nchidof_x_nudof *= scale;
	    			...
	    			*(fMaterial_Params[kLambda]+fMaterial_Params[kTau])*dFYdScol1;
	    			...
	    			scale =-1*scale_const*Jp*(1/fConst)*(fMaterial_Params[kKappa]-fMaterial_Params[kSigma_const])
	    			*(fMaterial_Params[kMu]+fMaterial_Params[kSigma_const])*dFYdScol1;
	    			fTemp_matrix_nchidof_x_nudof *= scale;
...
//    fV3.Dimension(n_sd_x_n_sd_x_n_sd);
    ...
    EPSI_tr.Dimension(n_sd,n_sd);
    EPSI.Dimension(n_sd,n_sd);
    ...
    while (NextElement())
    {
	    ...
	    for (l=0; l < fNumIP_displ; l++)
	    {
         ...
         fState_variables_n_Elements_IPs(e,l*kNUM_FMATERIAL_STATE_TERMS+kDelgamma)=0.0;
			...
			fChip_n_IPs.SetRow(l,fIdentity_matrix);


	    }
	    fFp_n_Elements_IPs.SetRow(e,fFp_n_IPs);
	    fCe_n_Elements_IPs.SetRow(e,fCe_n_IPs);
	    fChip_n_Elements_IPs.SetRow(e,fCe_n_IPs);
...
void FSMicromorphic3DT:: Form_fV2p()
{
    ...
    fTemp_matrix_nsd_x_nsd2.MultABCT(fFe,fS_SIGMA,fFe);
    fTemp_matrix_nsd_x_nsd.MultAB(fDeformation_Gradient_Inverse,fTemp_matrix_nsd_x_nsd2);
    ...
    for (int Kbar=0;Kbar<3;Kbar++)
    {
    	for(int l=0;l<3;l++)
        {
    		fV2p[row]=fTemp_matrix_nsd_x_nsd(Kbar,l);
            row++;
        }
    }
					...
					for(int k=0;k<3;k++)
					{
						for(int Kbar=0;Kbar<3;Kbar++)
						{
							for(int Lbar=0;Lbar<3;Lbar++)
							{
								...
								*fFe(k,Kbar)*fSPK(Kbar,Lbar)*fFe(l,Lbar);
							}
						}
					}
					row++;
    					...
    					for(int Kbar=0;Kbar<3;Kbar++)
    					{
    						for(int Lbar=0;Lbar<3;Lbar++)
    						{
    							for(int Nbar=0;Nbar<3;Nbar++)
    							{
    								...
    								*fSPK(Kbar,Lbar)*fFe(l,Lbar)*fFe(i,Nbar)*fFp_inverse(M,Nbar);
    							}
    						}
    					}
    				}
    						...
    						for(int Lbar=0;Lbar<3;Lbar++)
    						{
    							for(int Mbar=0;Mbar<3;Mbar++)
    							{
    								for(int Nbar=0;Nbar<3;Nbar++)
    								{
    									...
    									*fSPK(Kbar,Lbar)*fFe(l,Lbar)*fdFYdS(Mbar,Nbar)*fFe(i,Nbar)*fFp_inverse(M,Mbar);
    								}
    							}
    						}
    					}
    						...
    						for(int Lbar=0;Lbar<3;Lbar++)
    						{
    							for(int Mbar=0;Mbar<3;Mbar++)
    							{
    								for(int Nbar=0;Nbar<3;Nbar++)
    								{
    									...
    									*fSPK(Kbar,Lbar)*fFe(l,Lbar)*fdFYdS(Mbar,Nbar)*fFe(i,Mbar)*fFp_inverse(M,Nbar);
    								}
    							}
    						}
    					}
    			...
    			for(int Kbar=0;Kbar<3;Kbar++)
    			{
    				for(int Lbar=0;Lbar<3;Lbar++)
    				{
    					for(int k=0;k<3;k++)
    					{
    						...
    						*fSPK(Kbar,Lbar)*fFp_inverse(M,Lbar);
    					}
    				}
    			}
    			row=row+3;
    		...
    		for(int A=0; A<3;A++)
    		{
    			for(int l=0;l<3;l++)
    			{
    				...
    				for(int Abar=0;Abar<3;Abar++)
    				{
    					for(int Bbar=0;Bbar<3;Bbar++)
    					{
    						I4p_9(row,col)+=fTemp_matrix_nsd_x_nsd(A,l)*fFe(i,Abar)*fChip_inverse(M,Bbar);
    					}
    				}
    				row++;
    			}
    		...
    		for(int A=0; A<3;A++)
    		{
    			for(int l=0;l<3;l++)
    			{
    				...
    				for(int Abar=0;Abar<3;Abar++)
    				{
    					for(int Bbar=0;Bbar<3;Bbar++)
    					{
    						I4p_10(row,col)+=fTemp_matrix_nsd_x_nsd(A,l)*fFe(i,Bbar)*fChip_inverse(M,Abar);
    					}
    				}
    				row++;
    			}
...
void FSMicromorphic3DT :: Form_IIJp_1()
{
	...
	int col=0;
	IIJp_1=0.0;
	fTemp_matrix_nsd_x_nsd.MultABCT(fFe,fS_SIGMA,fFe);
	for(int M=0;M<3;M++)
	{
		for(int i=0;i<3;i++)
		{
...
void FSMicromorphic3DT:: Form_II2e_1()
{
	...
	int col=0;
	II2e_1=0.0;
	fTemp_matrix_nsd_x_nsd.MultABCT(fFp_inverse,fS_SIGMA,fFe);
	...
	for(int M=0;M<3;M++)
	{
		for(int m=0;m<3;m++)
		{
			...
			{
				//summation over the same term stars here
				II2e_1(row,col)+=fTemp_matrix_nsd_x_nsd(m,l);
				row=row+3;
			}
			col++;
		}
...
//	fTemp_matrix_nsd_x_nsd2.MultABC(fdGdS_n_transpose,fFp_n,fFp_inverse);
    ...
    fTemp_matrix_nsd_x_nsd.MultABCT(fTemp_matrix_nsd_x_nsd2,fS_SIGMA,fFe);
			...
			for(int l=0;l<3;l++)
			{
				for(int m=0;m<3;m++)
				{
    				...
    				for(int Mbar=0;Mbar<3;Mbar++)
    				{
    					for(int Nbar=0;Nbar<3;Nbar++)
    					{
    						II3e_1(row,col)+=fTemp_matrix_nsd_x_nsd(m,l)*fdFYdS(Mbar,Nbar)*fChie(i,Mbar)*fFp_inverse(M,Nbar);
    					}
    				}
    				row++;
    			}
...
void FSMicromorphic3DT:: Form_II4e_1()
{
	...
	fTemp_matrix_nsd_x_nsd.MultABCT(fFe,fS_SIGMA,fFp_inverse);
	...
	for(int M=0;M<3;M++)
    {
    	for(int l=0;l<3;l++)
    	{
			...
			for(int m=0;m<3;m++)
			{
				II4e_1(row,col)+=fTemp_matrix_nsd_x_nsd(m,M);
				row++;
    		}
    		col++;
    	}
...
void FSMicromorphic3DT:: Form_II4p_2()
{
	...
	fTemp_matrix_nsd_x_nsd.MultATBC(fdGdS_n,fFp_n,fFp_inverse);
    fTemp_matrix_nsd_x_nsd2.MultABC(fFe,fCe_n_inverse,fTemp_matrix_nsd_x_nsd);
    fTemp_matrix_nsd_x_nsd.MultABCT(fFe,fS_SIGMA,fTemp_matrix_nsd_x_nsd2);
