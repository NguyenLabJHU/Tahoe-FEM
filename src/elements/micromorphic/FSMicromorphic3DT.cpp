...
/*                Form_deformation_gradient_tensor();
             ...
             }//this loop end is the iConstitutiveModelType loop end
...
void FSMicromorphic3DT::Form_Second_Piola_Kirchhoff_SPK()
{
   ...
   scale=(fMaterial_Params[kLambda]+fMaterial_Params[kTau])*trE;
   SPK*=scale;
	...
	for(int k=0;k<3;k++)
	{
		for(int a=0;a<3;a++)
		{
			...
			for(int l=0;l<3;l++)
			{
				...
				I1_1(row,col)+=fTemp_matrix_nsd_x_nsd(k,l);
				row=row+3;
			}
			col++;
		}
...
void FSMicromorphic3DT:: Form_I1_2()
{
    ...
    for(int m=0;m<3;m++)
    {
    	for(int k=0;k<3;k++)
    	{
    		...
    		for(int l=0;l<3;l++)
    		{
    			I1_2(row,col)+=fTemp_matrix_nsd_x_nsd(m,l);
    			row=row+3;
    		}
        	col++;
    	}
...
void FSMicromorphic3DT:: Form_I1_3()
{
    ...
    for(int m=0;m<3;m++)
    {
    	for(int l=0;l<3;l++)
    	{
    		...
    		for(int k=0;k<3;k++)
    		{
    			I1_3(row,col)+=fTemp_matrix_nsd_x_nsd(k,m);
    			row++;
    		}
        	col++;
    	}
				...
				for(int k=0;k<3;k++)
				{
					...
					for(int K=0;K<3;K++)
					{
						for(int L=0;L<3;L++)
						{
							for(int M=0;M<3;M++)
							{
								I1_4(row,col)+=fDeformation_Gradient(k,K)*fDeformation_Gradient(n,M)*fDeformation_Gradient(i,M)*fIdentity_matrix(K,L)
			...
			for(int l=0;l<3;l++)
			{
				for(int k=0;k<3;k++)
				{
					...
					for(int K=0;K<3;K++)
					{
						for(int L=0;L<3;L++)
						{
							I1_5(row,col)+=fDeformation_Gradient(k,K)*fDeformation_Gradient(m,K)
					...
					for(int K=0;K<3;K++)
					{
						for(int L=0;L<3;L++)
						{
							I1_6(row,col)+=fDeformation_Gradient(k,K)*fDeformation_Gradient(i,K)
										  *fDeformation_Gradient(m,L)*fDeformation_Gradient(l,L);
						}
					}
				row++;
			}
