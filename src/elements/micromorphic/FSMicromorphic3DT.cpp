    ...
    */
    ...
    if (bStep_Complete)
    {
        ...
        //for (l=0; l < fNumIP_micro; l++)
        {
            ...
            out_variable.CopyIn(6,fTemp_nine_values);
            /*
...
/*                Form_H1_matrix();//output:H1 vctor & Mnplus1 tensor
                ...
                Pint_2_temp*=scale_const*J;//the sign is taken into account when forming H2 vector.
                Pint_2+=Pint_2_temp;

/*                Form_H3_matrix();//output H3 vector
                NCHI.MultTx(H3,Pint_3_temp);
                Pint_3_temp*=scale_const*fMaterial_Params[kRho_0];
                Pint_3+=Pint_3_temp;
*/
                ...
                fCauchy_stress_IPs.SetRow(IP,Sigma);
...
/*                fTemp_matrix_nchidof_x_nchidof.MultATBC(NCHI,RChi_1,NCHI);
                scale = -1*scale_const*J*(fMaterial_Params[kKappa]-fMaterial_Params[kSigma_const]);
                fTemp_matrix_nchidof_x_nudof *= scale;
                // accumulate
                ...
                scale = -1*scale_const*J*(fMaterial_Params[kNu]-fMaterial_Params[kSigma_const]);
                fTemp_matrix_nchidof_x_nudof *= scale;
                // accumulate
...
*/
 ...
 ////buraya bak gerekirse mathematicada yap
...
*/
/*                fTemp_matrix_nchidof_x_nudof.MultABCT(NCHI_Tr,R_gradu,fIota_temp_matrix);
                scale = scale_const*J;
                fTemp_matrix_nchidof_x_nudof *= scale;
                // accumulate
                fH2_14 += fTemp_matrix_nchidof_x_nudof;*/
...
/*    Counter_IPs_el_n.Dimension(n_el,fNumIP_displ);
    ...
    R_gradu.Dimension(n_sd_x_n_sd,n_sd_x_n_sd);
 ...
 //   GRAD_NCHI_Phi.Dimension(n_sd_x_n_sd_x_n_sd,n_en_micro_x_n_sd_x_n_sd);
    ...
    fH2_14.Dimension(n_en_micro*n_sd_x_n_sd,n_en_displ_x_n_sd);
   ...
   //  RHS.Dimension(9);
     ...
     Sigma6_1.Dimension(3,3);
     Sigma6_2.Dimension(3,3);
     Sigma6_3.Dimension(3,3);
     Sigma6_4.Dimension(3,3);
    ...
    for(int l=0;l<3;l++)
     {
         for(int k=0;k<3;k++)
         {
                 Sigma(l,k)+=2*fIdentity_matrix(l,k)*(fMaterial_Params[kKappa]);
                 for(int K=0;K<3;K++)
                 {
                     Sigma(l,k)+=(-ChiN[l][K]*Chi[K][k]-Fn_m(k,K)*Finv_m(K,l))*fMaterial_Params[kKappa];
                 }
          }
      }
    ...
    for(int l=0;l<3;l++)
     {
         for(int k=0;k<3;k++)
         {
                 Sigma(l,k)+=2*fIdentity_matrix(l,k)*(fMaterial_Params[kNu]);
                 for(int K=0;K<3;K++)
                 {
                     Sigma(l,k)+=(-ChiN[k][K]*Chi[K][l]-Fn_m(l,K)*Finv_m(K,k))*fMaterial_Params[kNu];
                 }
          }
      }
*/
        	...
        	for(int i=0;n<3;n++)
        	{
        		...
        		for(int k=0;k<3;k++)
        		{
        			for(int l=0;l<3;l++)
        			{
        				...
        				for(int N=0;N<3;N++)
        				{
        					SigCurr(row,col)+=Sigma(l,k)*fGrad_disp_matrix(n,N)*Finv[N][i];
        				}
        				row++;
        			}
        		}
            	...
            	for(int l=0; l<= 2; l++)
            	{
            		for(int k=0;k<=2;k++)
            		{
            			...
            			for(int n=0;n<=2;n++)
            			{
            				for(int K=0;K<=2;K++)
            				{
            					Mm_14(row, col)+=-Mnplus1[k][l][m]*fGrad_disp_matrix(n,K)*Finv[K][i]*Finv[T][n];
            				}
            			}
            			row++;
            		}
 ...
 for(int m = 0; m<= 2; m++)
     {
         for(int k=0;k<=2;k++)
         {
             row = m;//row calculations start here
             for(int l = 0; l <= 2; l++)
                 {
                         //summation on the same term starts here
                             for(int K=0;K<=2;K++)
                             {
                                 Ru_8(row, col) =(Ru_8(row, col) +Fn[l][K]*Finv[K][k]);
                             }
                             row=row+3;
                 }
                 col++;
...
void FSMicromorphic3DT:: Form_R_gradu_matrix()
{
	R_gradu=0.0;
	int row=0;
	int col=0;
		...
		for(int k=0;k<3;k++)
		{
			...
			for(int m=0;m<3;m++)
			{
				for(int l=0;l<3;l++)
				{
					...
					for(int N=0;N<3;N++)
					{
						R_gradu(row,col)+=s_sigma(m,l)*fGrad_disp_matrix(n,N)*Finv[N][k];
					}
					row++;
				}
			}
...
void FSMicromorphic3DT::Extract_six_values_from_symmetric_tensor(const dMatrixT &fTensor,dArrayT& fTemp_nine_values)
{
    ...
    fTemp_nine_values[5]=fTensor(2,1);//sigma32
}
