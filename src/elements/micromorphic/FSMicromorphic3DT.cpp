			...
			for(int L=0;L<3;L++)
			{
				for(int l=0;l<3;l++)
				{
					for(int K=0;K<3;K++)
					{
						for(int T=0;T<3;T++)
						{
							grad_Nu[p][r][s]+=ChiN[p][L]*ChiInv[L][l]*GRAD_Chi[l][K][T]*ChiInv[K][r]*fDeformation_Gradient_Inverse(T,s);
						}
					}
				}
			}
	...
	for(int r=0;r<3;r++)
	{
		for(int s=0;s<3;s++)
		{
			...
			for(int K=0;K<3;K++)
			{
				for(int L=0;L<3;L++)
				{
					grad_Nu[p][r][s]+=-GRAD_ChiN[p][L][K]*fDeformation_Gradient_Inverse(K,s)*ChiInv[L][r];
				}
			}
		}
	}
