...
void SSQ2P1MF::SetGlobalShape(void)
{
  ...
  for (int i = 0; i < NumIP(); i++)
  {
    const dArray2DT& DNabar = fGradbar[i];
    dMatrixT& GradU = fGradU_List[i];
    GradU = 0.0;
    
    if (NumSD() == 2) 
    {
        double* pUx = fLocDisp.Pointer();
        double* pUy = fLocDisp.Pointer(nen);
        double* pDNx = DNabar(0);
        double* pDNy = DNabar(1);
        for (int a = 0; a < nen; a++)
        {
            GradU(0,0) += (*pDNx)*(*pUx);
            GradU(1,1) += (*pDNy)*(*pUy);
    ...
    else 
    {
        double* pUx = fLocDisp.Pointer();
        double* pUy = fLocDisp.Pointer(nen);
        double* pUz = fLocDisp.Pointer(2*nen);
        double* pDNx = DNabar(0);
        double* pDNy = DNabar(1);
        double* pDNz = DNabar(2);
        for (int a = 0; a < nen; a++)
        {
            GradU(0,0) += (*pDNx)*(*pUx);
            GradU(0,1) += (*pDNy)*(*pUx);
