/* $Id: ElasticHydrogelT.cpp,v 1.1 2012-01-10 19:02:13 xiaorui Exp $ */
...
ElasticHydrogelT::ElasticHydrogelT(void):
	ParameterInterfaceT("elastic_hydrogel"),
...
const dMatrixT& ElasticHydrogelT::c_ijkl(void)
{
   ...
   /*load the history variables*/
	...
	double fkappa==fPot->GetParameter("kappa");
...
const dSymMatrixT& ElasticHydrogelT::s_ij(void)
{
	...
	{
    	fSolidFraction = CalculatePhi(J,fSolidFraction_n);
...
void ElasticHydrogelT::DefiniteParameters(ParameterList& list) const
{
...
void ElasticHydrogelT::TakeParameterList(const ParameterListT& list)
{
	...
	if (!fPot) ExceptionT::GeneralFail(caller, "could not construct \"%s\"", eq_pot.Name().Pointer());			
...
double ElasticHydrogelT::CalculatePhi(const double J, double phi_n)
{ 
	...
	double fkappa=fPot->GetParameter("kappa");
	double res=R*fT*(log(1-phi)+phi+fchi*phi*phi)-fkappa*mu*phi(phi*J-1)-chempo;
	double error=sqrt(res*res);

	int iteration=0;
	while (tot>ctol && iteration <  maxiteration)
	{ 
		iteration ++;
		
		double K=R*fT*(1/(phi-1)+1+2*fchi*phi)-kappa*mu*(2*phi*J-1);
		doulbe dphi = -res/K;
		phi = phi + dphi;
		
		res=R*fT*(log(1-phi)+phi+fchi*phi^phi)-kappa*mu*phi(phi*J-1)-chempo;
		error=sqrt(res*res);
	}
