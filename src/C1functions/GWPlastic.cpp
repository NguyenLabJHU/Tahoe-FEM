/* $Id: GWPlastic.cpp,v 1.1 2003-06-03 16:32:12 rjones Exp $ */
#include "GWPlastic.h"
#include <math.h>
#include <iostream.h>
...
* constructors
*/
GWPlastic::GWPlastic(double POWER, double MU, double SIGMA):
...
void GWPlastic::ResetParameters(double POWER, double MU, double SIGMA)
{
	fP=POWER;	
}
...
* Returning values
*/
double GWPlastic::Function(double x) const
// Returns the area value (p = 1) ONLY.
{
	...
	if (fP==1.0)
	{
		if (fS==0)
		{
			cout << "\n*** Bad SIGMA value in GWPlastic.cpp.\n";
	...
	{
		cout << "\n*** Bad POWER value in GWPlastic.cpp.\n";
		throw ExceptionT::kBadInputValue;
	}
...
double GWPlastic::DFunction(double x) const
// Returns the load (p = 3/2) ONLY.
{
	...
	else if (fP==1.5)
	{
		...
		// use the identity: exp[-z] M[a,b,z] = M[b-a,b,-z]
		{
			...
			if (term[3]==0)
			{
				cout << "**Error! - Zero denominator in GWPlastic. **\n";
				throw ExceptionT::kBadInputValue;
			}
...
double GWPlastic::DDFunction(double x) const
