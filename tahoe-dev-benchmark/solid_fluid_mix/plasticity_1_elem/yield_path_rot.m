...
%sim=load('eldataout_1_plstr.txt');
sim=load('eldataout_1_plstr_elast.txt');
time=sim(:,1);
s11=(sim(:,2))/1000;
s22=(sim(:,3))/1000;
s33=(sim(:,4))/1000;
...
%compare for rotation%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%sim=load('eldataout_1_plstr_rot.txt');
sim=load('eldataout_1_plstr_rot_elast.txt');
time=sim(:,1);
s11=(sim(:,2))/1000;
s22=(sim(:,3))/1000;
s33=(sim(:,4))/1000;
...
e12=sim(:,13);
p_prime=(sim(:,16))/1000;
I1_prime_rot=3*p_prime;
sdev_sdev_rot=(sim(:,17))/(1e6);
sqrt_sdev_sdev_rot = sqrt(sdev_sdev_rot);
...
for i=1:length(time)
    sig = [s11(i) s12(i) s13(i) ;
           ...
           s13(i) s23(i) s33(i) ];
    lambda = eig(sig);
    s1_rot(i,1) = max(lambda);
    s2_rot(i,1) = lambda(2);
    s3_rot(i,1) = min(lambda);
end
...
for i=1:length(time)
    eps = [e11(i) e12(i) e13(i) ;
           ...
           e13(i) e23(i) e33(i) ];
    lambda = eig(eps);
    e1_rot(i,1) = max(lambda);
    e2_rot(i,1) = lambda(2);
    e3_rot(i,1) = min(lambda);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% stress strain
figure(1)
%plot(abs(e33),abs(s33-s11),'LineWidth',2)
%plot(abs(e33),abs(s33),'LineWidth',2)
...
plot(abs(e3),abs(s3),'-k',abs(e3_rot),abs(s3_rot),'--k','LineWidth',2)
xlabel('e3')
ylabel('s3')
...
%plot(I1f,snormf,'-k',I1f0,snormf0,'--k',I1g,snormg,'-.k',I1_prime,sqrt_sdev_sdev,'ok',I1_prime_rot,sqrt_sdev_sdev_rot,'.k','LineWidth',1)
plot(I1f,snormf,'-k',I1f0,snormf0,'--k',I1_prime,sqrt_sdev_sdev,'-.ok',I1_prime_rot,sqrt_sdev_sdev_rot,'-.xk','LineWidth',1)
xlabel('I1')
ylabel('s2J2')
%legend('Ff','Fmid','Fi','Gf','sim')
%legend('Ff','Fi','Gf','sim')
%legend('F','F0','Gf','sim','sim-rot')
legend('F','F0','no rotation','with rotation')
%axis([-20 0 0 2])
set(gca,'FontName','Helvetica','FontSize',16)
