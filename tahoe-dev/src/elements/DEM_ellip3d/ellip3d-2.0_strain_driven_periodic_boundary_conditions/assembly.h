...
//   // the top and bottom boundaries. 06/22/2015
  ...
  // periodic boundary conditions, strain driven using the second approach, i.e. only move boundary particles
  void unixialCompressionPB2(int   total_steps,		// the timestep used in FEM and DEM may be different, so control the total_steps to keep the time the same
	    REAL  topDisplacement,	// displacement of top boundary, positive downward
	    REAL  botDisplacement,  	// displacement of bottom boundary, positive upward
	    int   first_snapshot,	// the number of first snapshot
	    int   snapshots, 		// number of snapshots
  ...
  // in the triaxial compression, we give two parameters, one is the displacement of the top and bottom boundary walls, which is positive inward. The other one is the side pressure. In the simulation, we will try to approach the displacement for the top and bottom boundary while keep the side pressure as constant at the same time.
  void triaxialCompression(int   total_steps,		// the timestep used in FEM and DEM may be different, so control the total_steps to keep the time the same
	    REAL  displacement,	// displacement of the bottom and top boundary, positve inward, this is displacement for one side.
	    REAL  sigma_a,  	// side pressure
	    int   first_snapshot,	// the number of first snapshot
	    int   snapshots, 		// number of snapshots
	    int   interval,  		// print interval for progress file
	    const char* iniptclfile, 
