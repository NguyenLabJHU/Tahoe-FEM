...
namespace dem{
	...
	// bndry_type refers the type of the boundary, following the boundary type in the assembly.h
	...
	if( stretch > dem::Parameter::getSingleton().parameter["bondStretchLimit"] || stretch < -2.0*dem::Parameter::getSingleton().parameter["bondStretchLimit"] ){
	    isAlive = false;
	    return;	// do not need to calculate forces
	}

	// (3) calculate bond force and apply bond force to peri-particle and boundary
	REAL kn_periBndry = dem::Parameter::getSingleton().parameter["periYoung"];	// just in value
	REAL kt_periBndry = dem::Parameter::getSingleton().parameter["periYoung"];	// just for test, July 15, 2014
	...
	Vec fn = (bondn-initBondVec)*kn_periBndry;	// force is pointing from the projector to the peri-point
	Vec ft = bondt*kt_periBndry;	// force is pointing from the projector to the peri-point

	// apply forces to peri-point
	periPoint->addAccelerationByForce(-fn-ft);
	// apply forces to boundary
	// do not apply at present, since boundary force is only used to adjust the motion of boundaries. July 15, 2014
