...
namespace dem {
  ...
  private:
    ...
    ///////////////////////////////////// periDynamics part //////////////////////////////////////////
    //////////////////////////////////////////////////////////////////////////////////////////////////
    int nPeriParticle;	// number of PeriParticles in the domain, only for master cpu
...
//  int printInterval;	// print interval
    ...
    REAL maxHorizonSize;	// the maximum horizon size of all peri-points, for all cpus

    std::vector<periDynamics::PeriParticle*> allPeriParticleVecInitial;	// only for master cpu
	// in simulations, we have peri-points and dem sands, for convience of domain generation, we input a cuboid domain 
	// filling with peri-points, this particle vector stores all these peri-points. Then since we have sand particles also
	// in the same assembly, we need to remove those peri-points that are within the sand particles to generate the periParticleVec
	// that are used for the calculation. July 14, 2014
    std::vector<periDynamics::PeriParticle*> allPeriParticleVec;	// only for master cpu
    std::vector<periDynamics::PeriParticle*> periParticleVec;	// coordinates of all the particles in this cpu, local peri-points in current cpu
    std::vector<periDynamics::PeriParticle*> interfacePeriParticleVec;
    std::vector<periDynamics::PeriParticle*> fixedPeriParticleVec;	// this is the peri-points that are inside the rigid inclusion, fix these peri-points
    std::vector<periDynamics::PeriParticle*> outerfacePeriParticleVec;	// this is only for the hollow spherical example
//  std::vector<periDynamics::PeriParticle*> bottomBoundaryVec;	// particles that are in the bottom boundary
//  std::vector<periDynamics::PeriParticle*> topBoundaryVec;	// particles that are in the bottom boundary
//  std::vector<periDynamics::PeriParticle*> cubicTopBoundaryVec;	// particles that are in the top boundary of the cubic
//  std::vector<periDynamics::PeriBond*> totalBondVec;	// all the Bonds in the domain

    // for all cpus, these bonds, including boundary bonds, peri-bonds, and peri-DEM-bonds will
    ...
    std::vector<PeriDEMBond*> periDEMBondVec;	// the bonds that between the sand particle and 
						// the peri-points that are near to this particle, in each cpu   

    std::vector<periDynamics::PeriParticle *> rperiParticleX1, rperiParticleX2; // r stands for received
    ...
    std::vector<periDynamics::PeriParticle *> rperiParticleX1Y1, rperiParticleX1Y2, rperiParticleX1Z1, rperiParticleX1Z2; 
    std::vector<periDynamics::PeriParticle *> rperiParticleX2Y1, rperiParticleX2Y2, rperiParticleX2Z1, rperiParticleX2Z2; 
    std::vector<periDynamics::PeriParticle *> rperiParticleY1Z1, rperiParticleY1Z2, rperiParticleY2Z1, rperiParticleY2Z2; 
    std::vector<periDynamics::PeriParticle *> rperiParticleX1Y1Z1, rperiParticleX1Y1Z2, rperiParticleX1Y2Z1, rperiParticleX1Y2Z2; 
    std::vector<periDynamics::PeriParticle *> rperiParticleX2Y1Z1, rperiParticleX2Y1Z2, rperiParticleX2Y2Z1, rperiParticleX2Y2Z2; 
    std::vector<periDynamics::PeriParticle *> recvPeriParticleVec;  // received particles per process
    std::vector<periDynamics::PeriParticle *> mergePeriParticleVec;

    std::vector<periDynamics::PeriParticle*> bottomBoundaryVec;	// particles that are in the bottom boundary
    std::vector<periDynamics::PeriParticle*> frontBoundaryVec;	// particles that are in the front y boundary
    std::vector<periDynamics::PeriParticle*> leftBoundaryVec;	// particles that are in the left x boundary

    std::vector<periDynamics::PeriParticle*> topBoundaryInnerVec;	// particles that are in the bottom boundary
    std::vector<periDynamics::PeriParticle*> topBoundaryEdgeVec;	// particles that are in the bottom boundary
    std::vector<periDynamics::PeriParticle*> topBoundaryCornerVec;	// particles that are in the bottom boundary

    std::vector<periDynamics::PeriBond*> recvPeriBondVec;	// peri-bonds between recvPeriParticle in each cpu
    std::vector<periDynamics::PeriBond*> periBondVec;		// peri-bonds between periParticleVec in each cpu
...
//  int Uxindex[5];  // particle index along the x direction.[y = 0, z = 0]
  ...
  public:
    Assembly()
      ...
      vibraTimeStep(0), impactTimeStep(0), nPeriParticle(0)
      {}
...
//    cubicTopBoundaryVec.clear();
  ...
  ///////////////////////////////////// periDynamics part //////////////////////////////////////////
  //////////////////////////////////////////////////////////////////////////////////////////////////
  int getnPeriParticle() const {return nPeriParticle;} // getnPeriParticle - returns number of peri-particles

  void setInitIsv();		
  void initialPeriDynamics(const char* );	// initial - initializes the velocity, displacement and acceleration 
				// @param const char* - name of the input file 
  void prescribeEssentialBoundaryCondition(const int);	// apply the fixed boundary condition on the bottom particles
  void solve(const char*);	// solve - solve this problem
				// @param const char* - output file name for tecplot visualization
  void runFirstHalfStep();	// run step1 and step2 in Page 5 of Houfu's notes, equations (15) and (16)
  void runSecondHalfStep();	// run step3 in page 5 of Houfu's notes, equation (17)
  void constructNeighbor();	// neibhor - searches and constructs the particle neighborlists
  void findRecvPeriBonds();	// find peri-bonds between periParticleVec and recvPeriParticleVec
  void findPeriDEMBonds();	// find sand-peri bonds in each cpu, i.e. periParticleVec and ParticleVec
  void clearPeriDEMBonds();
  void eraseBrokenPeriDEMBonds();
  void readPeriDynamicsData(const char *);	// readData - reads controlling parameters, particle positions and mesh connectivities
				// @param char * - reference of the input file name
  void writeMesh(const char *);	// writeMesh - outputs the mesh, used for problem checking
				// @param char * - reference of the output file name 
  void writeMeshCheckVolume(const char *);	// writeMeshCheckVolume - outputs the mesh and volume, will be used for volume comuptation checking
						// @param char * - reference of the output file name 
  void writeParticleTecplot(std::ofstream &, const int) const;	// writeParticleTecplot - outputs the information of all particles, for tecplot visualization
  ...
  void printPeriDomainSphere(const char*) const;	// print stress in spherical coordinates

  void constructPeriMatrix();		// construct Matrix members in periParticleVec
  void constructRecvPeriMatrix();	// construct Matrix members in recvPeriParticleVec, construction here since currently 
					// the pointer array in Matrix cannot be transfered well between cpus
  void calcDeformationGradient();	// calcDeformationGradient - calculates the deformation gradient for each peri-particle
  void calcHorizonSize();	// calcHorizonSize - calculates the horizon size for each peri-particle
  void calcParticleVolume();	// calcParticleVolume - calculates the particle volume associated with each particle
  void calcParticleKinv();	// calculate the inverse of K for all particles	
  void calcRecvParticleKinv();	// calculate the inverse of K for all recvPeriParticles	
  void calcParticleStress();	// calculate the Cauchy Stress for all particles
  void calcParticleAcceleration();	// calculate the acceleration for all particles		
  void checkBondParticleAlive();	// for each particle, check the state(alive or not) of each surrounding bond;
  ...
  void removeInsidePeriParticles();	// delete those peri-points that are inside sand particles
  void constructBoundarySandPeriBonds();	// construct boundary bonds, sand bonds, July 14, 2014
  void applyCoupledForces();	// check if peri-boundary bonds and peri-dem bonds are still alive, July 15, 2014
				// if so, apply coupled forces to peri-points and sand particles/boundaries
  void applyCoupledBoundary();	// this is used to test the coupled force model, October 10, 2014
