...
namespace dem {
  ...
  Vec Particle::globalToLocalPrev(Vec input) const {
    Vec lmn, local;
    lmn = vcos(getPrevDirecA()); local.setX(lmn * input); // l1,m1,n1
    lmn = vcos(getPrevDirecB()); local.setY(lmn * input); // l2,m2,n2
    lmn = vcos(getPrevDirecC()); local.setZ(lmn * input); // l3,m3,n3
    return local;
  }
  ...
  Vec Particle::localToGlobalPrev(Vec input) const {
    Vec lmn, global;
    lmn = vcos( Vec(prevDirecA.getX(),prevDirecB.getX(),prevDirecC.getX()) ); global.setX(lmn * input); // l1,l2,l3
    lmn = vcos( Vec(prevDirecA.getY(),prevDirecB.getY(),prevDirecC.getY()) ); global.setY(lmn * input); // m1,m2,n3
    lmn = vcos( Vec(prevDirecA.getZ(),prevDirecB.getZ(),prevDirecC.getZ()) ); global.setZ(lmn * input); // n1,n2,n3
    return global;
  }
  ...
  void Particle::update() {
    ...
    else if (getType() == 10) { // special case 10: pull out a DEM particle in peri-domain, prescribed constant velocity
      currPos = prevPos + currVeloc * timeStep;
    }
