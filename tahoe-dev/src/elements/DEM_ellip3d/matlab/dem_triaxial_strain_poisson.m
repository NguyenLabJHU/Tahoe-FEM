function dem_triaxial_stress_strain(fileToRead1)
...
%poisson = abs( (epsilon_v - epsilon_z) ./ epsilon_z );
poisson = abs( (epsilon_x + epsilon_y) ./ epsilon_z );
...
forceH = plot(epsilon_z, epsilon_v, 'b', 'LineWidth', 3); 
grid on;
grid minor;
pbaspect([1 1 1]);
...
subplot(1,2,2)
%forceH = plot(accruedTime, traction_z1, 'r--o', accruedTime, traction_z2, 'm--', accruedTime, traction_x1, 'b-x', accruedTime, traction_x2, 'k-p','LineWidth', 1); 
forceH = plot(epsilon_z, poisson, 'b', 'LineWidth', 3);
grid on;
grid minor;
pbaspect([1 1 1]);
title('Poisson ratio');
...
options.Format = 'png';
hgexport(fh, strcat(fileToRead1, '_poisson.png'), options);
