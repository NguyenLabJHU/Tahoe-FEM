function dem_impulse_force_energy(fileToRead1)
...
subplot(1,1,1)
[AX,H1,H2] = plotyy(accruedTime, [normal_z1, normal_z2], accruedTime, [transEnergy, rotatEnergy, kinetEnergy]);
grid on;
grid minor;
pbaspect(AX(1), [1 1 1]);
pbaspect(AX(2), [1 1 1]);
...
title('Force and energy response to a top impulse');
xlabel('Step'); 
set(get(AX(1), 'Ylabel'), 'String', 'Force (N)');
set(get(AX(2), 'Ylabel'), 'String', 'Energy (J)');
%set(AX(1), 'xlim', [0, 3.5e-4]);
set(AX(2), 'xlim', [0, 4000]);
lh=legend('bottomForce', 'topForce', 'transEnergy', 'rotatEnergy', 'kinetEnergy'); % , 'location', 'best'
set(lh, 'box', 'off');
