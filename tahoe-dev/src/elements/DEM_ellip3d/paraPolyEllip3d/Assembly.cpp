...
namespace dem {
  ...
  void Assembly::tuneMassPercent()
  {
	...
	for (int i = massPercent.size()-1; i >= 0 ; --i) { // do not use size_t for descending series
	  if ( (*itr)->getMaxRadius() <= massSize[i] )
  ...
  void Assembly::readParticle(const char *inputParticle) {	// July 3, 2015
    ...
    ifs >> particleNum;
    std::string str;
    ifs >> str >> str >> str >> str >> str >> str >> str >> str >> str >> str >> str	
	>> str >> str >> str >> str >> str >> str >> str >> str >> str >> str >> str
	>> str >> str >> str >> str >> str >> str >> str >> str >> str >> str;	// newly added three str's are for aminus, bminus, cminus
  ...
  void Assembly::printParticle(const char *str) const {	// July 3, 2015
    std::ofstream ofs(str);
	...
	<< std::setw(OWID) << "type"
	<< std::setw(OWID) << "a_plus"
	<< std::setw(OWID) << "a_minus"
	<< std::setw(OWID) << "b_plus"
	<< std::setw(OWID) << "b_minus"
	<< std::setw(OWID) << "c_plus"
	<< std::setw(OWID) << "c_minus"
    ...
    for (it = allParticleVec.begin(); it != allParticleVec.end();++it)  {
	  ...
	  << std::setw(OWID) << (*it)->getAplus()
	  << std::setw(OWID) << (*it)->getAminus()
	  << std::setw(OWID) << (*it)->getBplus()
	  << std::setw(OWID) << (*it)->getBminus()
	  << std::setw(OWID) << (*it)->getCplus()
	  << std::setw(OWID) << (*it)->getCminus();
  ...
  void Assembly::findContact() { // various implementations
	...
	for (std::size_t j = i + 1; j < num2; ++j){
	  ...
	  if ( ( vfabs(v - u) < particleVec[i]->getMaxRadius() + mergeParticleVec[j]->getMaxRadius())
