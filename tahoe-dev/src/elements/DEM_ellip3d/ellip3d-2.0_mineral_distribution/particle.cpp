...
particle::particle(int id, int tp, vec dim, vec position, vec dirca, vec dircb, vec dircc, REAL yng, REAL poi, REAL den)
...
//    density=Gs*1.0e3;
    ...
    mass=density*volume;
    J=vec(mass*0.2*(b*b+c*c),mass*0.2*(a*a+c*c),mass*0.2*(a*a+b*b));
    inContact=false;
    GlobCoef();
}
...
particle::particle(int id,int tp, vec dim, vec position, vec dirca, vec dircb, vec dircc, vec velocity, vec omga, REAL yng, REAL poi, REAL den)
  :ID(id), type(tp), young(yng), poisson(poi) {
...
REAL particle::getTransEnergy() const{
    return mass*pow(vfabs(curr_velocity),2)*0.5;
}
...
REAL particle::getRotatEnergy() const{
    ...
    return J.getx()*pow(curr_local_omga.getx(),2)*0.5 +
	   J.gety()*pow(curr_local_omga.gety(),2)*0.5 +
	   J.getz()*pow(curr_local_omga.getz(),2)*0.5;
}
...
void particle::planeRBForce(plnrgd_bdry<particle>* plb,
	...
	*/
	...
	vec veloc2 = getCurrVelocity() + getCurrOmga()*((pt1+pt2)*0.5-getCurrPosition());
	...
	addMoment(((pt1+pt2)*0.5-curr_position)*CntDampingForce);
	...
	if (BDRYFRIC != 0){
	    ...
	    vec RelaDispInc = (curr_velocity+curr_omga*((pt1+pt2)*0.5-curr_position))*TIMESTEP;  
		...
		vec RelaVel = curr_velocity + curr_omga*((pt1+pt2)*0.5-curr_position);  
	    ...
	    */
	    ...
	    addForce(TgtForce);
	    addMoment(((pt1+pt2)*0.5-curr_position)*TgtForce); 
...
vec particle::cylinderRBForce(int bdry_id, const cylinder& S, int side){
	...
	addForce(NormalForce);
	addMoment(((pt1+pt2)*0.5-getCurrPosition())*NormalForce);	    
