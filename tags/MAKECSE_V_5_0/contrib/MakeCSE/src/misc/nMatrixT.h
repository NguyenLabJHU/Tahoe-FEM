...
/*
 * created      : PAK (05/24/96)
 * last modified: PAK (08/11/98)
 */
...
class nMatrixT: public nArrayT<nTYPE>
{
  public:
	...
	/* control flags */
	enum SymmetryFlag {kWhole = 0,
                   kUpperOnly = 1};

	enum AssemblyMode {kOverwrite = 0,
                      kAccumulate = 1};
	...
	void CopyBlock(const ArrayT<int>& rc, nMatrixT<nTYPE>& block) const;
		//block must be square
	...
	int Cols(void) const;
  	...
  	void ShallowCopy(const nMatrixT& RHS);
	...
	void WriteRow(int row, const nArrayT<nTYPE>& vec); 
	void WriteRow(int row, const nTYPE& value); 
	void WriteCol(int col, const nArrayT<nTYPE>& vec); 
	void WriteCol(int col, const nTYPE& value); 
	...
	/* I/O operators */
	friend istream& operator>>(istream& in, nMatrixT<nTYPE>& matrix)
	{
	...
	friend ostream& operator<<(ostream& out, const nMatrixT<nTYPE>& matrix)
	{
		int width = OutputWidth(matrix.Pointer(), out);	
		for (int j = 0; j < matrix.fRows; j++)	
		{
			if (j > 0) out << '\n';
			for (int i = 0; i < matrix.fCols; i++)
...
//TEMP - ArrayT<int> does not support Max, Min
//	if (rc.Min() < 0 || 
//	    rc.Max() >= ((fRows > fCols) ? fRows : fCols) ) throw eOutOfRange;
#endif
...
template <class nTYPE> 
inline void nMatrixT<nTYPE>::ShallowCopy(const nMatrixT& RHS)
{
...
/* dimension check */
	...
	/* byte copy */
	memcpy(col.Pointer(),(*this)(colnum),sizeof(nTYPE)*fRows);
}
...
/* writing to rows/columns */
template <class nTYPE> 
void nMatrixT<nTYPE>::WriteRow(int row, const nArrayT<nTYPE>& vec)
{
	/* dimension check */
#if __option (extended_errorcheck)
...
template <class nTYPE> 
void nMatrixT<nTYPE>::WriteRow(int row, const nTYPE& value)
{
	nTYPE* pcol = Pointer() + row;	
	for (int i = 0; i < fCols; i++)
	{
...
template <class nTYPE>
void nMatrixT<nTYPE>::WriteCol(int col, const nArrayT<nTYPE>& vec)
{
/* dimension check */
#if __option (extended_errorcheck)
...
template <class nTYPE>
void nMatrixT<nTYPE>::WriteCol(int col, const nTYPE& value)
{
	nTYPE* pcol = (*this)(col);	
	for (int i = 0; i < fRows; i++)
		*pcol++ = value; 
