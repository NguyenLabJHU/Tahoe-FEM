...
void TecPlotOutputT::WriteGeometry(void)
{
  ...
  for (int e=0; e < fElementSets.Length(); e++)
	...
	iArrayT nodes_used;
	nodes_used.ShallowCopy(fElementSets[e]->NodesUsed());
...
void TecPlotOutputT::WriteOutput(double time, int ID, const dArray2DT& n_values, const dArray2DT& e_values)
{
  ...
  dArray2DT local_nvars (nodes_used.Length(), n_values.MinorDim());
  local_nvars.RowCollect (nodes_used, n_values);
  tec.WriteData (out, local_nvars);
