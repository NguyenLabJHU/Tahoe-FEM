...
{
 public:
  enum VariableType { kScalarElemental,
		      kVectorElemental,
		      kScalarNodal,
		      kVectorNodal };
	...
	int WriteConnectivityHeader (ostream& fgeo, GeometryT::GeometryCode code, int numelems, int numelemnodes) const;
	...
	void WriteVariableLabels (ostream& fvar, const ArrayT<StringT>& labels, const ArrayT<StringT>& filenames, const ArrayT<EnSightT::VariableType>& t) const;
	void WriteTime (ostream& fvar, int sequence, int start, int increment, const ArrayT<double>& timesteps) const;

	void GetElementName (StringT& fElementName, int& num_output_nodes, GeometryT::GeometryCode geocode) const;

	// read case file
	bool CaseFile (ifstream_x& in, StringT& geofile) const;
	bool ReadVariableSection (ifstream_x& in, AutoArrayT<StringT>& nlabels, AutoArrayT<StringT>& elabels, AutoArrayT<bool>& nvector, AutoArrayT<bool>& evector, bool filename) const;
	bool ReadTimeSection (ifstream_x& in, int& start, int& increment, dArrayT& timesteps) const;
	...
	void ReadConnectivity (istream& in, iArray2DT& conn, iArrayT& map, bool mapgiven, GeometryT::GeometryCode& code) const;
 ...
 private:
	...
	bool GeometryCode (GeometryT::GeometryCode& code, const StringT& name, int& num_elems) const;
