#!/usr/bin/perl -w
#
# $Id: MakeConfigMakeFile.pl,v 1.2 2002-12-08 17:33:51 paklein Exp $
#
...
open(HEADER, "$config_header_file");
...
if (-e $config_make_file) {
	...
	print "creating output file root: $config_make_file\n";
}
...
# -# the symbol in $out_root.h file will be ENABLE_[OPTION]
FIN
...
while ($line = <IN>) {
	...
	if ($line !~ /^#/ && $line =~ /[a-zA-Z0-9]/ ) {
		...
		# read description
			...
			print OUT "# $line\n";
			print OUT "# This option must be set in conjunction with #define ENABLE_$opt_root\n";
			print OUT "# in $out_root.h. */\n";

			# scan header file for symbol
			$symbol = "ENABLE_".$opt_root;
			$found = 0;
