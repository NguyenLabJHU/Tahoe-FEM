...
/* la_dgetrf.f -- translated by f2c (version 19960611).
...
/* Subroutine */ int dgetrf(m, n, a, lda, ipiv, info)
integer *m, *n;
doublereal *a;
...
/*        Use unblocked code. */
	...
	dgetf2(m, n, &a[a_offset], lda, &ipiv[1], info);
    } else {
...
/*           singularity. */
	    ...
	    i__3 = *m - j + 1;
	    dgetf2(&i__3, &jb, &a[j + j * a_dim1], lda, &ipiv[j], &iinfo);
