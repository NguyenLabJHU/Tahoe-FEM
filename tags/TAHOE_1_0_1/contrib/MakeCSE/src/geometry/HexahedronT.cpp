/* $Id: HexahedronT.cpp,v 1.2 2002-09-30 20:52:43 sawimme Exp $ */
...
void HexahedronT::EvaluateShapeFunctions(const dArrayT& coords, dArrayT& Na) const
{
	...
	if (coords.Length() != 3 ||
	        Na.Length() != fNumNodes) throw eSizeMismatch;
	if (fNumNodes != kNumVertexNodes && 
	    fNumNodes != 20) throw eGeneralFail;
#endif
	...
	double s = coords[1];
	double t = coords[2];
	if (r < -1.0 || r > 1.0) throw eOutOfRange;
	if (s < -1.0 || s > 1.0) throw eOutOfRange;
	if (t < -1.0 || t > 1.0) throw eOutOfRange;
	...
	dArray2DT& DNa) const
{
	...
	if (coords.Length() != 3 ||
	        Na.Length() != fNumNodes ||
	     DNa.MajorDim() != 3 ||
	     DNa.MinorDim() != fNumNodes) throw eSizeMismatch;
	if (fNumNodes != kNumVertexNodes && 
	    fNumNodes != 20) throw eGeneralFail;
#endif
...
void HexahedronT::SetExtrapolation(dMatrixT& extrap) const
{
	...
	/* dimension checks */
	if (numnodes < 8 || numnodes > 20) throw eGeneralFail;
