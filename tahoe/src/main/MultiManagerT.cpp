/* $Id: MultiManagerT.cpp,v 1.1.2.1 2003-10-16 12:56:14 paklein Exp $ */
...
void MultiManagerT::Initialize(InitCodeT)
{
	...
	if (n1 != n2)
		ExceptionT::GeneralFail(caller, "number of groups must match: %d != %d",
			n1, n2);
...
out << "\nupdate = \n" << update.wrap(10) << endl;
	...
	/* project coarse scale solution to the fine */
	fCoarse->ProjectField(bridging_field, *(fFine->NodeManager()), order);

	/* interpolate fine scale solution to the coarse */
