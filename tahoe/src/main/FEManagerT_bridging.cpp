/* $Id: FEManagerT_bridging.cpp,v 1.19.4.6 2004-05-17 05:14:14 paklein Exp $ */
	...
	double smoothing, double k2, int nip)
{
	...
	for (int i = 0; i < bonds.MajorDim(); i++) /* one bond family at a time */ {
		...
		/* number unknown bond densities */
		bond_densities_i_eq_man.SetMajorDimension(overlap_cell_i.Length(), false);
		CountIPBonds(overlap_cell_i, bond_densities_i_eq);
		bond_densities_i_eq = 1; //TEMP - all active
		int num_eq = 0;
		for (int j = 0; j < overlap_cell_i.Length(); j++)
			for (int k = 0; k < bond_densities_i_eq.MinorDim(); k++)
				if (bond_densities_i_eq(j,k) > 0)
					bond_densities_i_eq(j,k) = ++num_eq;
				else
					bond_densities_i_eq(j,k) = -1; /* inactive */
		...
		while (iter++ < max_iter && error > abs_tol && error/error_0 > rel_tol && error/error_0 < div_tol) {
			...
			try {
				...
				for (int j = 0; j < bond_densities_i_eq.Length(); j++) {
					int eq = bond_densities_i_eq[j] - 1;
					if (eq >= 0) /* active */
						rhs[eq] = -df_dp_i[j];
				}
