 ...
 *====================================================================*/
#ifndef lint
static char rcsid[] = "$Id: az_tools.c,v 1.1.1.1 2001-01-30 20:59:01 paklein Exp $";
#endif
 ...
 ******************************************************************************/
    ...
    if (data_org[AZ_matrix_type] == AZ_MSR_MATRIX) {
      for (i = 0 ; i < length ; i++ ) newvec[i] = vector[ update_index[i] ] ;
    }
...
#ifdef __MWERKS__ /* added because MacMPI does not support MPI_ANY_SOURCE */
	int  i2;
	int* recv_list_lengths;
	int  max_num_recv_neighbors;
	int* all_recv_lists;
	int* precv;
	int num_send_count;
#endif
...
/*** resolving number of neighbors by posting with unspecified source/destination ***/
/* MacMPI does not support MPI_SOURCE_ANY for irecv */
#ifdef __MWERKS__ 
	/* allocate space for receive list lengths */
	recv_list_lengths = (int*) calloc((unsigned) nprocs, sizeof(int));
	if (recv_list_lengths == NULL) {
		(void) fprintf(stderr, "%sERROR: Not enough dynamic space.\n", yo);
		exit(-1);
	}

	/* collect length of all receive lists */
	md_allgather((char*) &num_recv_neighbors, (char*) recv_list_lengths, 1*sizeof(int));

	/* determine max number of expected receives */
	max_num_recv_neighbors = 0;
	for (i = 0; i < nprocs; i++)
		 max_num_recv_neighbors = (recv_list_lengths[i] > max_num_recv_neighbors) ? 
		 	recv_list_lengths[i] : 
		 	max_num_recv_neighbors;

	if (max_num_recv_neighbors > 0)
	{
		/* allocate space for receiving all recv lists */
		all_recv_lists = (int *) calloc((unsigned) max_num_recv_neighbors*nprocs, sizeof(int));
		if (all_recv_lists == NULL) {
			(void) fprintf(stderr, "%sERROR: Not enough dynamic space.\n", yo);
			exit(-1);
		}

		/* collect all receive lists */
		md_allgather((char*) recv_list, (char*) all_recv_lists, max_num_recv_neighbors*sizeof(int));

		/* search through all receive lists to see who needs a send */
		num_send_count = 0;
		precv = all_recv_lists;
		for (i = 0; i < nprocs; i++)
		{
			for (i2 = 0; i2 < recv_list_lengths[i]; i2++)
				if (precv[i2] == proc)
				{
					/* already full */
					if (num_send_count == num_send_neighbors - 1) {
						(void) fprintf(stderr, "%sERROR: found too many send partners.\n", yo);
						exit(-1);
					}
					send_list[num_send_count++] = i;
					i2 = recv_list_lengths[i];
				}
			precv += nprocs;
		}

		/* clean up */
		free((char *) all_recv_lists);
	}

	/* clean up */
	free((char *) recv_list_lengths);
#else
  /* first post receives */
  length = 0;
