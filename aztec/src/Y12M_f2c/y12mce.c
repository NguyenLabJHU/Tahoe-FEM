...
/* y12mce.f -- translated by f2c (version 19960611).
...
/* Subroutine */ int y12mce(n, z__, a, snr, nn, rnr, nn1, pivot, b, ha, iha, 
	aflag, iflag, ifail)
integer *n, *z__;
real *a;
...
/*  information which is necessary to begin the elimination is stored. */
	...
	*ifail = 2;
    ...
    if (aflag[1] < (float)1.) {
	aflag[1] = (float)1.0005;
    }
    if (aflag[3] < (float)1e5) {
	aflag[3] = (float)1e5;
    }
    if (aflag[4] < (float)0.) {
	aflag[4] = -aflag[4];
