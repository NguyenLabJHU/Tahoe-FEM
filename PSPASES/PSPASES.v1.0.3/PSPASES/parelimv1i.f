...
C/*                                                                           */
C/*****************************************************************************/
C/* $Id: parelimv1i.f,v 1.1.1.1 2004-10-07 16:05:26 paklein Exp $ */
C/*****************************************************************************/

      subroutine PARELIMV1(wmem,vbuf_s,hbuf_s,vbuf_r,hbuf_r,supinds,
     ...
     5           wmemsize,locinds,N,prof,dfopts,ifopts,comm)
      ...
      if (newsup .eq. 1) then
        ...
        if (diagproc .eq. 1) then
          if (myid .ne. mydown) then
            call mpi_send(supinds(rsuptr),nrows,MPI_INTEGER,
     +                    mydown,myid,comm,ierr)
          end if
        else
          msgtype = MPI_ANY_TAG
          call mpi_recv(locinds,INDXBUFSIZE,MPI_INTEGER,myup,msgtype,
     +                  comm,mpistat,ierr)
          call mpi_get_count(mpistat,MPI_INTEGER,nbytes,ierr)
          msgtype = mpistat(MPI_TAG)
          if (mydown .ne. msgtype) then
            call mpi_send(locinds,nbytes,MPI_INTEGER,mydown,msgtype,
     +                    comm,ierr)
          end if
...
125           newlocf = locf + newrank * (ldf + 1)
            ...
            do while (i .lt. nclim)
              ...
              do while (i+newrank .lt. nclim)
                if (supinds(i+newrank).gt.inode .or.
     1           supinds(i+newrank).ne.locinds(indxbufptr+jj+newrank))
     2          goto 135
                newrank = newrank + 1
              end do
135           i = i + newrank
...
50        continue
     ...
     1         myright,myid,comm,req(1),ierr)
          ...
          if (myid .ne. mydown) then
            ...
            call mpi_isend(vbuf_s,ishft(rank*nrows,3),MPI_BYTE,mydown,
...
70          newlocf = newlocf + newrank * (ldf + 1)
            ...
            prof(2,nprof) = newrank
            prof(3,nprof) = k
            call dsyrk('L','N',newrank,rank,-1.d0,wmem(uptr),ldf,1.d0,
     1           wmem(locf),ldf)
            i = i + newrank
            j = j + newrank
            call dgemm('N','T',nrlim-j,newrank,rank,-1.d0,
     1           wmem(uptr+newrank),ldf,wmem(uptr),ldf,1.d0,
     2           wmem(locf+newrank),ldf)
            uptr = uptr + newrank
          end do
        else
...
90          i = i + newrank 
            ...
            prof(4,nprof) = jj + 1
            call dgemm('N','T',nrlim-j,newrank,rank,-1.d0,
     1           wmem(uptr),ldf,vbuf_r(jj+1),ldb,1.d0,wmem(locf),ldf)
            j = j + k
            newlocf = newlocf + k
            uptr = uptr + k
          end do
     ...
     +                     comm,req(2),ierr)
          ...
          do 120 i = 1, nprof
            ...
            call dsyrk('L','N',newrank,rank,-1.d0,hbuf_r(uptr),ldb,
     1           1.d0,wmem(prof(1,i)),ldf)
            call dgemm('N','T',j,newrank,rank,-1.d0,
     1           hbuf_r(uptr+newrank),ldb,hbuf_r(uptr),ldb,
     2           1.d0,wmem(prof(1,i)+newrank),ldf)
120       continue
